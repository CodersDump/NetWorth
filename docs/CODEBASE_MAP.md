...
| `public_walkins` |  | 845 | — |
| `_settlement_rows` | group_id | 865 | Per (month, year, slot): the exact math from the Calculations sheet. |
| `summary` | group_id, scope_slots | 979 | — |
| `insights` | group_id | 1001 | Per-member monthly economics, ghosts, and walk-in conversion. |
| `_response` | status_code, body_dict | 1241 | — |
...
### Frontend (`frontend/js/app.js` — 8257 LOC, flat global script, ~357 functions)
...
- `loadFinanceExpenses()` — L5567
- `resetExpenseEdit()` — L5605
- `addFinanceExpense()` — L5612
- `loadFinanceMembers()` — L5631
- `markMembersDirty()` — L5735
- `recalcMembers()` — L5742
- `renderBulkRosterList()` — L5754
- `bulkAddFromRoster()` — L5768
- `copyPreviousMonthMembers()` — L5783
- `addFinanceMember()` — L5819
- `loadFinanceWalkins()` — L5840
- `addFinanceWalkin()` — L5867
- `loadFinanceInsights()` — L5905
- `copyDuesForWhatsApp()` — L5919
- `pad(s, w)` — L5936
- `padL(s, w)` — L5937
- `line(n, o, r, p)` — L5938
- `done()` — L5948
- `fallbackCopy(text, cb)` — L5954
- `renderInsights()` — L5963
- `saveFinanceSettings()` — L6053
- `loadPublicWalkins()` — L6092
- `loadReviewDay()` — L6175
- `reviewOrderChanged()` — L6219
- `renderReviewList()` — L6225
- `applyReviewOrder()` — L6286
- `updateAuthUI()` — L6314
- `hiddenNow(id, btn)` — L6342

**Live scoring inside tournaments**  (from L6421)
- `refreshMySession(statusElId)` — L6427
- `setStatus(msg)` — L6428
- `openAchievementsModal()` — L6456
- `closeAchievementsModal()` — L6465
- `openAuthModal()` — L6466
- `closeAuthModal()` — L6467
- `showAuthView(view)` — L6468
- `setAuthSession(session, user, opts = {})` — L6476
- `closeCompleteProfileModal()` — L6496
- `openCompleteProfileModal()` — L6497
- `showCompleteProfileMode(mode, preselectPlayerId)` — L6512
- `populateClaimPicker(preselectPlayerId)` — L6520
- `submitClaimProfile()` — L6544
- `closeCompleteProfileModal()` — L6584
- `sanitizeNickname(raw)` — L6590
- `editDistance(a, b)` — L6595
- `checkForExistingPlayer(name, typedNickname, statusEl)` — L6617
- `submitCompleteProfile()` — L6672
- `finishRequestAndSignOut(message)` — L6748
- `doLogin()` — L6754
- `doNewPassword()` — L6807
- `doSignup()` — L6818
- `doConfirmSignup()` — L6835
- `doResendConfirmCode()` — L6866
- `doForgotPassword()` — L6877
- `doConfirmForgotPassword()` — L6892
- `doLogout()` — L6904
- `restoreSession()` — L6948
- `restoreTabFromHash()` — L6992
- `addManualTeamRow()` — L7052
- `collectManualTeams()` — L7088
- `loadTournamentGroupOptions()` — L7101
- `loadTournamentParticipantsChecklist()` — L7110
- `updateParticipantsCount()` — L7140
- `collectTournamentParticipants()` — L7152
- `loadTournamentsList()` — L7156
- `submitTournamentCreation(payload)` — L7163
- `collectAllEntities(t)` — L7322
- `getAllTeamEntities(t)` — L7338
- `renderTeamCompositionBars(t, containerId)` — L7356
- `populateSubstitutionSection(t)` — L7391
- `updateSubOldPlayerOptions()` — L7402
- `formatGames(games)` — L7491
- `applyTournamentViewMode()` — L7498
- `matchTotals(match)` — L7504
- `truncateBracketName(name, maxChars = 22)` — L7512
- `renderBracketView(t)` — L7517
- `renderTournament(t)` — L7633
- `generateTournamentRecap(t)` — L7807
- `downloadTournamentImage()` — L7839
- `loadImg(src)` — L7866
- `sideVisuals(side)` — L7876
- `drawCard(x, y, w, match, isFinal)` — L7883
- `drawAvatars(ctx, x, y, side, isWinner)` — L7929
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L7948
- `roundRect(ctx, x, y, w, h, r)` — L7976
- `copyTournamentRecap()` — L7986
- `item_has_third_place(t)` — L7997
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L8001
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L8007
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L8026
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L8032
- `submitThirdPlaceScore(tournamentId)` — L8051
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L8057
- `getTournamentLiveLog(matchKey)` — L8080
- `tournamentLivePoint(matchKey, side, target)` — L8085
- `tournamentUndoPoint(matchKey, target)` — L8094
- `updateTournamentLiveDisplay(matchKey, target)` — L8100
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L8118
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L8127
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L8136
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L8145
- `applyTheme(theme)` — L8238
