...
### Frontend (`frontend/js/app.js` — 12502 LOC, flat global script, ~495 functions)
...
**Quick record: tap mode + voice stack + shared queue**  (from L2204)
...
- `nwOpenSessionAddPanel()` — L2439
- `nwSessionAddMember(body)` — L2452
- `nwSessionRemoveMember(playerId)` — L2488
- `nwTapSlotsPerTeam()` — L2510
- `nwTapRefreshAvatarGrid()` — L2514
- `nwTapToggleAvatar(playerId)` — L2594
- `nwTapPlayerName(id)` — L2603
- `nwTapRenderTeams()` — L2608
- `nwTapPointsToWin()` — L2644
- `nwTapFinalScore()` — L2645
- `nwTapRenderQuickScore()` — L2649
- `nwTapRenderManual()` — L2681
- `nwStackVoiceNote(said)` — L2811
- `nwSaveQueue()` — L2863
- `nwRenderQueue()` — L2867
- `groupName(gid)` — L2873
- `nwQueueEditItem(id)` — L2901
- `nwQueueRemoveItem(id)` — L2940
- `nameFor(pid)` — L3044
- `showMatchOutcome(ok, message)` — L3114

**Unsaved-match safety net**  (from L3126)
- `savePendingMatch(payload, meta)` — L3133
- `loadPendingMatch()` — L3137
- `clearPendingMatch()` — L3140
- `handleSessionExpired()` — L3147
- `ensureRestoreHost()` — L3170
- `offerPendingMatchRestore()` — L3179

**Game log & CSV export**  (from L3212)
- `loadGameLog()` — L3214
- `gameLogGoto(p)` — L3264
- `renderGameLog()` — L3266
- `matchPermissions(m)` — L3322
- `matchGroupLabel(m)` — L3344
- `canActOnMatchDirectly(m)` — L3362
- `requestMatchChange(matchId, type, label, groupId, extra)` — L3369
- `editMatch(matchId, groupId)` — L3393
- `opts(sel)` — L3402
- `pickers(team, prefix)` — L3404
- `close()` — L3425
- `editMatchScore(matchId, currentScoreA, currentScoreB, e)` — L3450
- `deleteMatch(matchId, encLabel, groupId)` — L3486
- `downloadCSV(filename, rows)` — L3518
- `loadRankings()` — L3552
- `gp(p)` — L3578
- `fetchRatingHistory(playerId)` — L3623
- `loadVisiblePlayers(opts = {})` — L3634

**Profile card customization**  (from L3709)
- `resolveBannerId(id)` — L3772
- `bgCss(id, url)` — L3846
- `updatePageBackground()` — L3851
- `applyPageBackground(player)` — L3862
- `renderProfileCardBanner(player)` — L3869
- `toggleHeaderMenu()` — L3913
- `openSettingsModal()` — L3927
- `loadFinanceAccessList()` — L3941
- `opt(v, label)` — L3959
- `setGroupFinanceRole(groupId, playerId, role)` — L3974
- `setGroupMemberRole(groupId, playerId, role, wasRole, isSelf)` — L3990
- `setFinanceRole(playerId, role)` — L4006
- `closeSettingsModal()` — L4017
- `renderSettingsPickers(player)` — L4021
- `swatch(field, id, css, selected)` — L4030
- `submitClaimRequest()` — L4059
- `checkApprovalStatus()` — L4084
- `recomputeNow()` — L4097
- `loadAppSettings()` — L4112
- `setXpPublic(value)` — L4143
- `setVoiceEnabled(value)` — L4158
- `setInstantCreate(value)` — L4173

**Quests**  (from L4185)
- `loadQuests()` — L4186
- `_renderQuestRow(q)` — L4195
- `_hdr(t)` — L4219
- `claimQuest(questId)` — L4227
- `loadQuestsAdmin()` — L4243
- `saveQuest()` — L4264
- `deleteQuest(questId)` — L4286

**Achievements**  (from L4297)
- `badgeSvg(tier, glyph)` — L4313
- `loadMyAchievements()` — L4337
- `claimAchievement(achievementId)` — L4372
- `loadAchievementsAdmin()` — L4399
- `editAchievement(achievementId)` — L4457
- `cancelAchievementEdit()` — L4474
- `saveAchievement()` — L4489
- `deleteAchievement(achievementId)` — L4516
- `revokeAchievementClaim(achievementId)` — L4531
- `seedStarterAchievements()` — L4575

**Store & events admin**  (from L4616)
- `loadStore()` — L4617
- `catOf(i)` — L4642
- `cardHtml(i)` — L4647
- `buyStoreItem(itemId)` — L4679
- `onStoreImagePick(input)` — L4693
- `loadStoreAdmin()` — L4701
- `onStoreTypeChange()` — L4729
- `onStoreEffectChange()` — L4741
- `uploadStoreImage(file)` — L4757
- `saveStoreItem()` — L4774
- `deleteStoreItem(itemId)` — L4824
- `loadEventsAdmin()` — L4835
- `editEvent(e)` — L4857
- `saveEvent()` — L4865
- `deleteEvent(eventId)` — L4888
- `refreshEventBanner()` — L4900
- `loadClaimAudit()` — L4917
- `relinkAccount(usernameEnc, presetPlayerId)` — L4984
- `unlinkAccount(usernameEnc)` — L4992
- `unlinkAndStrip(usernameEnc, playerId)` — L4997
- `_claimAuditAction(bodyObj)` — L5002
- `loadUnconfirmedUsers()` — L5013
- `deleteUnconfirmedUser(username, email)` — L5040
- `loadClaimRequests()` — L5053
- `decideClaimRequest(requestId, action, requestType)` — L5108
- `escapeHtml(s)` — L5146

**Image uploads**  (from L5151)
- `resizeImage(file, kind)` — L5163
- `isAnimatedImage(file)` — L5198
- `uploadCardImage(kind, fileInput)` — L5210
- `imageSrc(key)` — L5265
- `loadStoreCatalogOnce()` — L5271
- `renderStoreCosmeticStrip(kind, player)` — L5281
- `renderUploadStrip(kind, player)` — L5305
- `vsPlayerVisual(pid, snapshot)` — L5337
- `vsAvatarHtml(v, isWinner)` — L5353
- `teamBanner(side)` — L5370
- `gameScore(game, side)` — L5381
- `renderVsCard(idsA, idsB, opts = {})` — L5387
- `won(side)` — L5392
- `vsSideIds(side)` — L5417
- `setMyCardField(field, value)` — L5427

**Profile bundle / cards / charts**  (from L5495)
- `loadProfileBundle(playerId)` — L5495
- `renderTieredCard(icon, name, unit, tiers, currentValue)` — L5610
- `renderBinaryCard(icon, name, desc, achieved, detail)` — L5638
- `resetRatingZoom()` — L5710
- `loadProfileRatingChart(playerId)` — L5716
- `loadProfilePartnershipsAndRadar(playerId)` — L5802
- `loadProfileHeadToHead(playerId)` — L5851
- `loadProfileWithPartner(playerId)` — L5875
- `partnerGamesGoto(p)` — L5907
- `renderPartnerGames()` — L5909
- `skeletonHTML(lines = 3)` — L5942
- `showProfileSkeletons()` — L5949
- `renderXpPanel(player)` — L5962
- `xpForLevel(n)` — L5970
- `updateHeaderCoins()` — L5996
- `loadProfile()` — L6008
- `refreshProfile()` — L6038
- `refreshProfileIfShowing(affectedPlayerIds)` — L6055
- `renderPartnerRadar(data, highlightTournament, svgId = 'rada)` — L6076
- `loadHistory()` — L6132
- `renderHistory(data)` — L6150
- `loadBadges()` — L6211
- `renderBadges(data)` — L6229
- `loadDiversity()` — L6262
- `renderDiversity(data)` — L6280
- `playerLabelById(playerId, fallbackName)` — L6301
- `playerLabelsById(playerIds, fallbackNames)` — L6305
- `loadHallOfFame()` — L6311
- `renderHallOfFame(data)` — L6333
- `loadAttendance()` — L6417
- `renderAttendance(data)` — L6436

**UPI payment card**  (from L6451)
- `refreshUpiCard()` — L6455
- `renderUpiCard()` — L6467
- `imageServiceFallback()` — L6489

**Finance tab (view-key + role gated)**  (from L6509)
- `xpVisible()` — L6517
- `applyFinanceRoleVisibility()` — L6523
- `refreshFinanceRoleForGroup()` — L6554
- `finQS(extra)` — L6567
- `financeBaseUrl()` — L6578
- `finPost(path, method, bodyObj)` — L6582
- `populateFinanceSlots(group)` — L6606
- `_rememberedFinance(key)` — L6629
- `_rememberFinance(key, val)` — L6633
- `restoreFinanceMonth()` — L6639
- `populateFinanceGroups()` — L6648
- `reloadFinanceForGroup()` — L6676
- `tryAutoFinanceUnlock()` — L6681
- `myFinanceGroups()` — L6711
- `populateMyDuesGroups()` — L6716
- `loadMyDues(groupId)` — L6734
- `manageGroupSlots(groupId)` — L6784
- `assignSlotMembers(groupId, slotEnc)` — L6802
- `transferGroupOwnership(groupId)` — L6830
- `setGroupPayee(groupId)` — L6849
- `requestFinanceAccess()` — L6872
- `financeUnlock()` — L6889
- `updateFinanceScopeNote(scopedTo)` — L6944
- `loadFinanceSummary()` — L6954
- `loadFinanceExpenses()` — L6994
- `resetExpenseEdit()` — L7032
- `addFinanceExpense()` — L7039
- `loadFinanceMembers()` — L7058
- `markMembersDirty()` — L7162
- `recalcMembers()` — L7169
- `renderBulkRosterList()` — L7181
- `bulkAddFromRoster()` — L7195
- `copyPreviousMonthMembers()` — L7210
- `addFinanceMember()` — L7246
- `resetWalkinEdit()` — L7272
- `suggestWalkinSessions()` — L7291
- `loadFinanceWalkins()` — L7323
- `addFinanceWalkin()` — L7375
- `loadFinanceInsights()` — L7408
- `copyDuesForWhatsApp()` — L7422
- `pad(s, w)` — L7439
- `padL(s, w)` — L7440
- `line(n, o, r, p)` — L7441
- `done()` — L7451
- `fallbackCopy(text, cb)` — L7457
- `copyInsightsTableAsImage()` — L7486
- `renderInsights()` — L7592
- `saveFinanceSettings()` — L7727
- `loadPublicWalkins()` — L7773

**Match review & reorder (SuperAdmin)**  (from L7861)
- `loadReviewDay()` — L7866
- `reviewOrderChanged()` — L7910
- `renderReviewList()` — L7916
- `applyReviewOrder()` — L7977
- `updateAuthUI()` — L8005
- `hiddenNow(id, btn)` — L8033
- `refreshMySession(statusElId)` — L8118
- `setStatus(msg)` — L8119
- `openAchievementsModal()` — L8147
- `closeAchievementsModal()` — L8156
- `openAuthModal()` — L8157
- `closeAuthModal()` — L8158
- `showAuthView(view)` — L8159
- `setAuthSession(session, user, opts = {})` — L8167
- `closeCompleteProfileModal()` — L8187
- `openCompleteProfileModal()` — L8188
- `showCompleteProfileMode(mode, preselectPlayerId)` — L8203
- `populateClaimPicker(preselectPlayerId)` — L8211
- `submitClaimProfile()` — L8235
- `closeCompleteProfileModal()` — L8275
- `sanitizeNickname(raw)` — L8281
- `editDistance(a, b)` — L8286
- `checkForExistingPlayer(name, typedNickname, statusEl)` — L8308
- `submitCompleteProfile()` — L8363
- `finishRequestAndSignOut(message)` — L8439
- `doLogin()` — L8445
- `doNewPassword()` — L8498
- `doSignup()` — L8509
- `doConfirmSignup()` — L8526
- `doResendConfirmCode()` — L8557
- `doForgotPassword()` — L8568
- `doConfirmForgotPassword()` — L8583
- `doLogout()` — L8595
- `restoreSession()` — L8639
- `restoreTabFromHash()` — L8683

**Tournaments**  (from L8717)
- `addManualTeamRow()` — L8758
- `collectManualTeams()` — L8794
- `loadTournamentGroupOptions()` — L8807
- `loadTournamentParticipantsChecklist()` — L8816
- `updateParticipantsCount()` — L8846
- `collectTournamentParticipants()` — L8858
- `loadTournamentsList()` — L8862
- `submitTournamentCreation(payload)` — L8869
- `submitManualDraftCreation(group_id, name)` — L8895
- `draftPlayerName(pid)` — L8943
- `draftEveryone(t)` — L8948
- `renderManualDraftTournament(t)` — L8954
- `fetchTournamentDetail(tournamentId)` — L9053
- `fetchAndRenderTournamentDetail(tournamentId)` — L9069
- `stopSchedulePolling()` — L9100
- `startSchedulePolling(tournamentId)` — L9105
- `isSchedulePollingActiveFor(tournamentId)` — L9115
- `schedulePollTick(tournamentId)` — L9117
- `renderDraftLeaderPicker(t)` — L9148
- `saveDraftLeaders(tournamentId)` — L9168
- `renderDraftPoolBoard(t)` — L9178
- `chip(pid)` — L9183
- `draftChipTapped(pid, ev)` — L9230
- `draftPoolColumnTapped(tournamentId, poolName)` — L9237
- `draftChipDragStart(ev, pid)` — L9244
- `draftPoolDragOver(ev)` — L9249
- `draftPoolDrop(ev, tournamentId, poolName)` — L9254
- `moveDraftPlayerToPool(tournamentId, poolName, playerId)` — L9263
- `putDraftPool(tournamentId, poolName, playerIds)` — L9283
- `addNewDraftPlayer(tournamentId, groupId)` — L9293
- `removeDraftPlayer(tournamentId, playerId)` — L9319
- `lockDraftPools(tournamentId)` — L9332
- `stopDraftPolling()` — L9362
- `startDraftPolling(tournamentId)` — L9367
- `isDraftPollingActiveFor(tournamentId)` — L9380
- `pollDraftStateTick(tournamentId)` — L9382
- `draftDecidedIds(draft)` — L9393
- `renderDraftStartAuctionPanel(t)` — L9402
- `startDraftAuction(tournamentId)` — L9411
- `renderDraftAuctionRoom(t)` — L9421
- `draftAssignEligibleLeaders(t, pool)` — L9436
- `draftAssignLeaderOptionsHtml(t, pool)` — L9446
- `updateDraftAssignLeaderOptions()` — L9452
- `renderDraftOrganizerAssignPanel(t)` — L9461
- `organizerAssignPlayer(tournamentId)` — L9486
- `renderDraftLiveStatusHtml(tournamentId, draftLike)` — L9507
- `updateDraftLiveStatus(tournamentId, draftLike)` — L9542
- `renderDraftQueuePicker(t)` — L9555
- `openDraftLot(tournamentId, playerId)` — L9582
- `closeDraftLot(tournamentId)` — L9590
- `skipDraftLot(tournamentId)` — L9600
- `renderDraftBidBox()` — L9610
- `draftBidBump(delta)` — L9623
- `submitDraftBid(tournamentId)` — L9630
- `renderDraftSquadsReview(t)` — L9659
- `renderSetSquadPairsPanel(t)` — L9679
- `generateCrossSquadGroups(tournamentId, status)` — L9742
- `saveSquadPairs(tournamentId, squadId, numGroups, slotsP)` — L9759
- `generateDraftSchedule(tournamentId)` — L9780
- `renderSquadRosterEditPanel(t, allowMove)` — L9801
- `renameSquadPrompt(tournamentId, squadId)` — L9867
- `moveSquadPlayer(tournamentId)` — L9884
- `toggleSquadSubNewPlayerFields(useNew)` — L9898
- `substituteSquadPlayer(tournamentId)` — L9915
- `draftSquadName(t, squadId)` — L9967
- `toggleDraftGroupOpen(name, detailsEl)` — L9989
- `toggleDraftSquadSection(key, detailsEl)` — L10002
- `renderDraftScheduleView(t)` — L10006
- `renderProjectedKnockout(t)` — L10103
- `renderRegenerateScheduleGroupPanel(t)` — L10125
- `regenerateDraftSchedule(tournamentId)` — L10149
- `renderSquadStandingsTable(standings, projection)` — L10176
- `computeLeaderboardRows(stats, t)` — L10215
- `tallyPair(side)` — L10252
- `squadSideField(tie, sid)` — L10314
- `decidingPairKey(tie, sid)` — L10315
- `podiumRank(row)` — L10373
- `renderPlayerTournamentStatsTable(stats, t)` — L10389
- `rowHtml(row)` — L10397
- `renderTieSection(title, ties, t, stageKind)` — L10442
- `renderTieCard(tie, t, stageKind)` — L10449
- `renderTieMatchRow(tie, m, idx, t, stageKind, iLeadA, iLead)` — L10481
- `draftTieMatchAdminControlsHtml(tournamentId, tieId, idx, stageKind, sid)` — L10667
- `cancelDraftTieMatch(tournamentId, tieId, matchIndex, stageKi)` — L10675
- `forfeitDraftTieMatch(tournamentId, tieId, matchIndex, stageKi)` — L10686
- `draftPlayerPickerHtml(tournamentId, tieId, matchIndex, members)` — L10697
- `opts(selected)` — L10703
- `pickTiePlayer(tournamentId, tieId, matchIndex, playerI)` — L10723
- `pickTiePlayerPair(tournamentId, tieId, matchIndex, squadId)` — L10735
- `submitDraftTieScore(tournamentId, tieId, matchIndex, stageKi)` — L10751
- `submitDraftTieScoreDirect(tournamentId, tieId, matchIndex, stageKi)` — L10768
- `collectAllEntities(t)` — L10934
- `getAllTeamEntities(t)` — L10950
- `renderTeamCompositionBars(t, containerId)` — L10968
- `populateSubstitutionSection(t)` — L11003
- `updateSubOldPlayerOptions()` — L11014
- `formatGames(games)` — L11103
- `applyTournamentViewMode()` — L11110
- `matchTotals(match)` — L11126
- `truncateBracketName(name, maxChars = 22)` — L11134
- `renderBracketView(t)` — L11139
- `renderDraftBracketGroupsPanel(t)` — L11267
- `renderDraftBracketView(t)` — L11303
- `renderTournament(t)` — L11434
- `generateTournamentRecap(t)` — L11609
- `downloadTournamentImage()` — L11641
- `loadImg(src)` — L11668
- `sideVisuals(side)` — L11678
- `drawCard(x, y, w, match, isFinal)` — L11685
- `drawAvatars(ctx, x, y, side, isWinner)` — L11731
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L11750
- `roundRect(ctx, x, y, w, h, r)` — L11778
- `downloadDraftShareImage()` — L11794
- `loadImg(src)` — L11801
- `sideAvatars(squadId)` — L11850
- `drawSide(side, sx, sy, isWinner)` — L11931
- `downloadDraftLeaderboardImage()` — L12012
- `loadImg(src)` — L12021
- `presetKeyFor(bannerCss)` — L12030
- `copyTournamentRecap()` — L12140
- `item_has_third_place(t)` — L12151
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L12155
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L12161
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L12198
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L12204
- `submitThirdPlaceScore(tournamentId)` — L12231
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L12237

**Live scoring inside tournaments**  (from L12264)
- `getTournamentLiveLog(matchKey)` — L12268
- `tournamentLivePoint(matchKey, side, target)` — L12273
- `tournamentUndoPoint(matchKey, target)` — L12282
- `updateTournamentLiveDisplay(matchKey, target)` — L12288
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L12306
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L12320
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L12329
- `finishDraftTieLiveGame(matchKey, tournamentId, tieId, matchInde)` — L12347
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L12356
- `activateTab(tabName)` — L12380
- `jumpToRecordMatch()` — L12477
- `applyTheme(theme)` — L12483
