...
### Frontend (`frontend/js/app.js` — 10682 LOC, flat global script, ~450 functions)
...
- `renderProjectedKnockout(t)` — L8591
- `renderRegenerateScheduleGroupPanel(t)` — L8613
- `regenerateDraftSchedule(tournamentId)` — L8637
- `renderSquadStandingsTable(standings, projection)` — L8664
- `renderPlayerTournamentStatsTable(stats, t)` — L8696
- `tallyPair(side)` — L8734
- `deriveGroups(members)` — L8749
- `squadSideField(tie, sid)` — L8767
- `decidingPairKey(tie, sid)` — L8768
- `pairRow(group, isHighlighted, tierColor)` — L8796
- `squadCard(sid)` — L8822
- `renderTieSection(title, ties, t, stageKind)` — L8867
- `renderTieCard(tie, t, stageKind)` — L8874
- `renderTieMatchRow(tie, m, idx, t, stageKind, iLeadA, iLead)` — L8893
- `draftPlayerPickerHtml(tournamentId, tieId, matchIndex, members)` — L9031
- `opts(selected)` — L9037
- `pickTiePlayer(tournamentId, tieId, matchIndex, playerI)` — L9057
- `pickTiePlayerPair(tournamentId, tieId, matchIndex, squadId)` — L9069
- `submitDraftTieScore(tournamentId, tieId, matchIndex, stageKi)` — L9085
- `submitDraftTieScoreDirect(tournamentId, tieId, matchIndex, stageKi)` — L9102
- `collectAllEntities(t)` — L9268
- `getAllTeamEntities(t)` — L9284
- `renderTeamCompositionBars(t, containerId)` — L9302
- `populateSubstitutionSection(t)` — L9337
- `updateSubOldPlayerOptions()` — L9348
- `formatGames(games)` — L9437
- `applyTournamentViewMode()` — L9444
- `matchTotals(match)` — L9460
- `truncateBracketName(name, maxChars = 22)` — L9468
- `renderBracketView(t)` — L9473
- `renderDraftBracketGroupsPanel(t)` — L9601
- `renderDraftBracketView(t)` — L9637
- `renderTournament(t)` — L9768
- `generateTournamentRecap(t)` — L9943
- `downloadTournamentImage()` — L9975
- `loadImg(src)` — L10002
- `sideVisuals(side)` — L10012
- `drawCard(x, y, w, match, isFinal)` — L10019
- `drawAvatars(ctx, x, y, side, isWinner)` — L10065
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L10084
- `roundRect(ctx, x, y, w, h, r)` — L10112
- `downloadDraftShareImage()` — L10128
- `loadImg(src)` — L10135
- `sideAvatars(squadId)` — L10184
- `drawSide(side, sx, sy, isWinner)` — L10265
- `copyTournamentRecap()` — L10321
- `item_has_third_place(t)` — L10332
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L10336
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L10342
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L10379
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L10385
- `submitThirdPlaceScore(tournamentId)` — L10412
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L10418
- `getTournamentLiveLog(matchKey)` — L10449
- `tournamentLivePoint(matchKey, side, target)` — L10454
- `tournamentUndoPoint(matchKey, target)` — L10463
- `updateTournamentLiveDisplay(matchKey, target)` — L10469
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L10487
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L10501
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L10510
- `finishDraftTieLiveGame(matchKey, tournamentId, tieId, matchInde)` — L10528
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L10537
- `activateTab(tabName)` — L10561
- `jumpToRecordMatch()` — L10657
- `applyTheme(theme)` — L10663
