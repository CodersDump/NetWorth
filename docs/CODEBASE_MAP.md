...
### Frontend (`frontend/js/app.js` — 11003 LOC, flat global script, ~456 functions)
...
- `podiumRank(row)` — L8875
- `renderPlayerTournamentStatsTable(stats, t)` — L8891
- `rowHtml(row)` — L8899
- `renderTieSection(title, ties, t, stageKind)` — L8944
- `renderTieCard(tie, t, stageKind)` — L8951
- `renderTieMatchRow(tie, m, idx, t, stageKind, iLeadA, iLead)` — L8983
- `draftTieMatchAdminControlsHtml(tournamentId, tieId, idx, stageKind, sid)` — L9169
- `cancelDraftTieMatch(tournamentId, tieId, matchIndex, stageKi)` — L9177
- `forfeitDraftTieMatch(tournamentId, tieId, matchIndex, stageKi)` — L9188
- `draftPlayerPickerHtml(tournamentId, tieId, matchIndex, members)` — L9199
- `opts(selected)` — L9205
- `pickTiePlayer(tournamentId, tieId, matchIndex, playerI)` — L9225
- `pickTiePlayerPair(tournamentId, tieId, matchIndex, squadId)` — L9237
- `submitDraftTieScore(tournamentId, tieId, matchIndex, stageKi)` — L9253
- `submitDraftTieScoreDirect(tournamentId, tieId, matchIndex, stageKi)` — L9270
- `collectAllEntities(t)` — L9436
- `getAllTeamEntities(t)` — L9452
- `renderTeamCompositionBars(t, containerId)` — L9470
- `populateSubstitutionSection(t)` — L9505
- `updateSubOldPlayerOptions()` — L9516
- `formatGames(games)` — L9605
- `applyTournamentViewMode()` — L9612
- `matchTotals(match)` — L9628
- `truncateBracketName(name, maxChars = 22)` — L9636
- `renderBracketView(t)` — L9641
- `renderDraftBracketGroupsPanel(t)` — L9769
- `renderDraftBracketView(t)` — L9805
- `renderTournament(t)` — L9936
- `generateTournamentRecap(t)` — L10111
- `downloadTournamentImage()` — L10143
- `loadImg(src)` — L10170
- `sideVisuals(side)` — L10180
- `drawCard(x, y, w, match, isFinal)` — L10187
- `drawAvatars(ctx, x, y, side, isWinner)` — L10233
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L10252
- `roundRect(ctx, x, y, w, h, r)` — L10280
- `downloadDraftShareImage()` — L10296
- `loadImg(src)` — L10303
- `sideAvatars(squadId)` — L10352
- `drawSide(side, sx, sy, isWinner)` — L10433
- `downloadDraftLeaderboardImage()` — L10514
- `loadImg(src)` — L10523
- `presetKeyFor(bannerCss)` — L10532
- `copyTournamentRecap()` — L10642
- `item_has_third_place(t)` — L10653
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L10657
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L10663
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L10700
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L10706
- `submitThirdPlaceScore(tournamentId)` — L10733
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L10739
- `getTournamentLiveLog(matchKey)` — L10770
- `tournamentLivePoint(matchKey, side, target)` — L10775
- `tournamentUndoPoint(matchKey, target)` — L10784
- `updateTournamentLiveDisplay(matchKey, target)` — L10790
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L10808
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L10822
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L10831
- `finishDraftTieLiveGame(matchKey, tournamentId, tieId, matchInde)` — L10849
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L10858
- `activateTab(tabName)` — L10882
- `jumpToRecordMatch()` — L10978
- `applyTheme(theme)` — L10984
