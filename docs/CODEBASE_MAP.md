...
| `delete_record_enforced` | record_type, record_id, event | 515 | Triple-gated: SuperAdmin identity + FINANCE_VIEW_KEY + the existing |
| `delete_record` | record_type, record_id, body, group_id | 538 | — |
| `get_settings` |  | 552 | — |
| `put_settings` | body | 561 | — |
| `public_upi` |  | 576 | The pay card is shown to guests (they pay walk-in fees), so the UPI |
| `my_settlement` | claims, group_id | 584 | A single member's own dues in a group: for every (month, slot) where |
| `public_walkins` |  | 648 | — |
| `_settlement_rows` | group_id | 668 | Per (month, year, slot): the exact math from the Calculations sheet. |
| `summary` | group_id | 753 | — |
| `insights` | group_id | 770 | Per-member monthly economics, ghosts, and walk-in conversion. |
| `_response` | status_code, body_dict | 970 | — |
...
### Frontend (`frontend/js/app.js` — 7305 LOC, flat global script, ~296 functions)
...
- `markMembersDirty()` — L4912
- `recalcMembers()` — L4919
- `renderBulkRosterList()` — L4931
- `bulkAddFromRoster()` — L4945
- `copyPreviousMonthMembers()` — L4960
- `addFinanceMember()` — L4996
- `loadFinanceWalkins()` — L5017
- `addFinanceWalkin()` — L5044
- `loadFinanceInsights()` — L5082
- `copyDuesForWhatsApp()` — L5096
- `pad(s, w)` — L5113
- `padL(s, w)` — L5114
- `line(n, o, r, p)` — L5115
- `done()` — L5125
- `fallbackCopy(text, cb)` — L5131
- `renderInsights()` — L5140
- `saveFinanceSettings()` — L5213
- `loadPublicWalkins()` — L5250
- `loadReviewDay()` — L5329

**Tournaments**  (from L5371)
- `reviewOrderChanged()` — L5373
- `renderReviewList()` — L5379
- `applyReviewOrder()` — L5428
- `updateAuthUI()` — L5455
- `hiddenNow(id, btn)` — L5472
- `openAuthModal()` — L5542
- `closeAuthModal()` — L5543
- `showAuthView(view)` — L5544
- `setAuthSession(session, user, opts = {})` — L5552
- `openCompleteProfileModal()` — L5569
- `showCompleteProfileMode(mode, preselectPlayerId)` — L5584
- `populateClaimPicker(preselectPlayerId)` — L5592
- `submitClaimProfile()` — L5616
- `closeCompleteProfileModal()` — L5656
- `sanitizeNickname(raw)` — L5662
- `editDistance(a, b)` — L5667
- `checkForExistingPlayer(name, typedNickname, statusEl)` — L5689
- `submitCompleteProfile()` — L5744
- `finishRequestAndSignOut(message)` — L5820
- `doLogin()` — L5826
- `doNewPassword()` — L5879
- `doSignup()` — L5890
- `doConfirmSignup()` — L5907
- `doResendConfirmCode()` — L5938
- `doForgotPassword()` — L5949
- `doConfirmForgotPassword()` — L5964
- `doLogout()` — L5976
- `restoreSession()` — L6020
- `restoreTabFromHash()` — L6054
- `addManualTeamRow()` — L6120
- `collectManualTeams()` — L6156
- `loadTournamentGroupOptions()` — L6169
- `loadTournamentParticipantsChecklist()` — L6178
- `updateParticipantsCount()` — L6208
- `collectTournamentParticipants()` — L6220
- `loadTournamentsList()` — L6224
- `submitTournamentCreation(payload)` — L6231
- `collectAllEntities(t)` — L6390
- `getAllTeamEntities(t)` — L6406

**Live scoring inside tournaments**  (from L6421)
- `renderTeamCompositionBars(t, containerId)` — L6424
- `populateSubstitutionSection(t)` — L6459
- `updateSubOldPlayerOptions()` — L6470
- `formatGames(games)` — L6559
- `applyTournamentViewMode()` — L6566
- `matchTotals(match)` — L6572
- `truncateBracketName(name, maxChars = 22)` — L6580
- `renderBracketView(t)` — L6585
- `renderTournament(t)` — L6701
- `generateTournamentRecap(t)` — L6875
- `downloadTournamentImage()` — L6907
- `loadImg(src)` — L6934
- `sideVisuals(side)` — L6944
- `drawCard(x, y, w, match, isFinal)` — L6951
- `drawAvatars(ctx, x, y, side, isWinner)` — L6997
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L7016
- `roundRect(ctx, x, y, w, h, r)` — L7044
- `copyTournamentRecap()` — L7054
- `item_has_third_place(t)` — L7065
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L7069
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L7075
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L7094
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L7100
- `submitThirdPlaceScore(tournamentId)` — L7119
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L7125
- `getTournamentLiveLog(matchKey)` — L7148
- `tournamentLivePoint(matchKey, side, target)` — L7153
- `tournamentUndoPoint(matchKey, target)` — L7162
- `updateTournamentLiveDisplay(matchKey, target)` — L7168
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L7186
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L7195
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L7204
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L7213
- `applyTheme(theme)` — L7286
