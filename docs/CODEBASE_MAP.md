...
### Frontend (`frontend/js/app.js` — 7278 LOC, flat global script, ~293 functions)
...
- `markMembersDirty()` — L4901
- `recalcMembers()` — L4908
- `renderBulkRosterList()` — L4920
- `bulkAddFromRoster()` — L4934
- `copyPreviousMonthMembers()` — L4949
- `addFinanceMember()` — L4985
- `loadFinanceWalkins()` — L5006
- `addFinanceWalkin()` — L5033
- `loadFinanceInsights()` — L5071
- `copyDuesForWhatsApp()` — L5085
- `done()` — L5098
- `fallbackCopy(text, cb)` — L5104
- `renderInsights()` — L5113
- `saveFinanceSettings()` — L5186
- `loadPublicWalkins()` — L5223
- `loadReviewDay()` — L5302

**Init & session restore**  (from L5346)
- `reviewOrderChanged()` — L5346
- `renderReviewList()` — L5352

**Tournaments**  (from L5371)
- `applyReviewOrder()` — L5401
- `updateAuthUI()` — L5428
- `hiddenNow(id, btn)` — L5445
- `openAuthModal()` — L5515
- `closeAuthModal()` — L5516
- `showAuthView(view)` — L5517
- `setAuthSession(session, user, opts = {})` — L5525
- `openCompleteProfileModal()` — L5542
- `showCompleteProfileMode(mode, preselectPlayerId)` — L5557
- `populateClaimPicker(preselectPlayerId)` — L5565
- `submitClaimProfile()` — L5589
- `closeCompleteProfileModal()` — L5629
- `sanitizeNickname(raw)` — L5635
- `editDistance(a, b)` — L5640
- `checkForExistingPlayer(name, typedNickname, statusEl)` — L5662
- `submitCompleteProfile()` — L5717
- `finishRequestAndSignOut(message)` — L5793
- `doLogin()` — L5799
- `doNewPassword()` — L5852
- `doSignup()` — L5863
- `doConfirmSignup()` — L5880
- `doResendConfirmCode()` — L5911
- `doForgotPassword()` — L5922
- `doConfirmForgotPassword()` — L5937
- `doLogout()` — L5949
- `restoreSession()` — L5993
- `restoreTabFromHash()` — L6027
- `addManualTeamRow()` — L6093
- `collectManualTeams()` — L6129
- `loadTournamentGroupOptions()` — L6142
- `loadTournamentParticipantsChecklist()` — L6151
- `updateParticipantsCount()` — L6181
- `collectTournamentParticipants()` — L6193
- `loadTournamentsList()` — L6197
- `submitTournamentCreation(payload)` — L6204
- `collectAllEntities(t)` — L6363
- `getAllTeamEntities(t)` — L6379
- `renderTeamCompositionBars(t, containerId)` — L6397

**Live scoring inside tournaments**  (from L6421)
- `populateSubstitutionSection(t)` — L6432
- `updateSubOldPlayerOptions()` — L6443
- `formatGames(games)` — L6532
- `applyTournamentViewMode()` — L6539
- `matchTotals(match)` — L6545
- `truncateBracketName(name, maxChars = 22)` — L6553
- `renderBracketView(t)` — L6558
- `renderTournament(t)` — L6674
- `generateTournamentRecap(t)` — L6848
- `downloadTournamentImage()` — L6880
- `loadImg(src)` — L6907
- `sideVisuals(side)` — L6917
- `drawCard(x, y, w, match, isFinal)` — L6924
- `drawAvatars(ctx, x, y, side, isWinner)` — L6970
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L6989
- `roundRect(ctx, x, y, w, h, r)` — L7017
- `copyTournamentRecap()` — L7027
- `item_has_third_place(t)` — L7038
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L7042
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L7048
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L7067
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L7073
- `submitThirdPlaceScore(tournamentId)` — L7092
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L7098
- `getTournamentLiveLog(matchKey)` — L7121
- `tournamentLivePoint(matchKey, side, target)` — L7126
- `tournamentUndoPoint(matchKey, target)` — L7135
- `updateTournamentLiveDisplay(matchKey, target)` — L7141
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L7159
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L7168
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L7177
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L7186
- `applyTheme(theme)` — L7259
