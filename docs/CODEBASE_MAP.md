...
| `compute_season_leaderboard` | season, items, k, min_games | 377 | Independent, season-scoped Elo ladder: everyone is seeded once at a |
| `_season_board_leaders` | season, items, k | 496 | Leaders for a season: sealed (frozen) if it has ended, else live. |
| `_season_badges_for` | player_id, leaders | 514 | A player's standing + earned badges on one season board. |
| `compute_player_season_summary` | player_id, items | 537 | Per-season standing + badges for one player, across started seasons. |
| `_quest_period` | quest | 558 | (bounds, claim_prefix, label) for a quest by scope. Season-scoped quests |
| `list_quests` | event | 573 | Returns this week's quests with the caller's progress and claim state. |
| `save_quest` | event | 616 | — |
| `delete_quest` | event | 646 | — |
| `claim_quest` | event | 659 | Player claims a completed quest's reward. Verified server-side against |
| `_load_achievements` |  | 735 | — |
| `list_achievements` | event | 740 | Every achievement, with the caller's live progress/claim state. |
| `save_achievement` | event | 788 | — |
| `delete_achievement` | event | 822 | — |
| `claim_achievement` | event | 835 | Player claims a completed achievement's reward. Progress is |
| `unclaim_achievement` | event | 890 | SuperAdmin-only corrective action (added 2026-08-29): reverses one |
| `_load_events` |  | 939 | — |
| `event_multiplier_for_date` | date_str, events | 943 | The XP multiplier active on a given match date (default 1.0). Pass a |
| `display_name` | player_item, fallback | 964 | Single source of truth for name formatting: 'Nickname (Real Name)' |
| `compute_comeback_bonus` | momentum | 981 | Extra rating-point bonus for the winning side, on top of the |
| `_is_valid_completed_game` | score_a, score_b, target | 995 | BWF-style badminton scoring: first to `target` points wins, but must lead |
| `_caller_claims` | event | 1012 | Claims API Gateway's Cognito Authorizer attaches to the request. |
| `_is_super_admin` | claims | 1018 | — |
| `_caller_may_edit_match` | claims, match | 1023 | Who may directly edit/delete a match (PUT/DELETE /matches/{id}): |
| `_can_view_profile` | claims, target_player_id | 1055 | SuperAdmin sees everyone. Anyone can view their own profile. A |
| `_requires_linked_member` | claims | 1075 | Signing up is not the same as being a member. Cognito self-signup is |
| `record_match_enforced` | event | 1098 | — |
| `_queue_item_out` | item | 1108 | Shapes one queue row for the API response - the frontend's shared- |
| `create_queue_item` | event | 1130 | Adds one not-yet-submitted match to its group's SHARED queue - |
| `list_queue` | event | 1183 | Every member of the group sees the SAME pending queue - polled by |
| `delete_queue_item` | queue_id, event | 1214 | Removes one queued item - either because it was just sent |
| `profile_view_enforced` | event | 1240 | Entry point for the isolated /profile-secure/{proxy+} catch-all. |
| `handler` | event, context | 1267 | — |
| `list_events` | event | 1350 | Public read - the frontend shows an active-event banner to everyone. |
| `save_event` | event | 1358 | SuperAdmin creates or updates an event (upsert by event_id). |
| `delete_event` | event | 1388 | — |
| `recompute_now` | event | 1401 | SuperAdmin-only: replay every match to rebuild ratings, XP, levels |
| `reorder_matches` | event | 1411 | Reorders a set of matches by reassigning their timestamps. |
| `record_match` | event | 1479 | — |
| `update_match` | match_id, event | 1520 | Fix a mis-entered score on an already-recorded standalone match. |
| `delete_match` | match_id, event | 1588 | Permanently delete a mis-recorded match - e.g. the wrong player was |
| `recompute_all_ratings` |  | 1608 | Elo is path-dependent - each match's rating change depends on the |
| `compute_momentum_stats` | point_log, winner | 1732 | Longest scoring streak per team, and how big a deficit the winner overcame. |
| `compute_adaptive_k` | pairing_count | 1779 | Higher K for a fresh/novel doubles pairing (each match together is |
| `get_pairing_count` | team_ids, exclude_match_id | 1795 | How many prior doubles matches has this exact 2-player team played |
| `_play_and_log` | match_type, team_a_ids, team_b_ids, score_a,  | 1815 | — |
| `list_matches` | event | 1912 | — |
| `compute_partnerships` | player_id, items | 2098 | For a given player, tally win/loss record with each doubles partner |
| `get_group_member_ids` | group_id | 2140 | The set of player_ids belonging to a group, used to filter WHO shows |
| `compute_attendance` | items, group_id_filter | 2151 | Per-player attendance/consistency: total matches, distinct calendar |
| `compute_hall_of_fame` | items, group_id_filter, hidden_ids | 2219 | Highlight stats computed from full chronological match history: |
| `compute_achievements` | player_id, matches, tournaments | 2548 | Milestone/tiered achievement progress for one player: total matches |
| `compute_top_opponents` | player_id, matches, top_n | 2689 | This player's win/loss record against every opponent they've ever |
| `compute_overall_record` | player_id, matches | 2731 | This player's total win/loss record, split by singles and doubles. |
| `compute_head_to_head` | player_id, opponent_id, matches | 2760 | One player's win/loss record specifically as an OPPONENT of another |
| `compute_with_partner` | player_id, partner_id, matches | 2792 | One player's win/loss record when partnered WITH another player on |
| `compute_recent_form` | player_id, matches, limit | 2843 | A player's last N matches, in chronological order (oldest to |
| `compute_diversity` | items, group_id_filter | 2900 | For every player: how concentrated their doubles partnerships are. |
| `compute_progress_history_summary` | scope_label, period_name | 2945 | Reads the permanent, locked-in weekly/monthly/yearly winner history |
| `compute_progress_badges` | items, group_id_filter | 3022 | For each of the last week/month/year: who improved their rating the |
| `compute_partner_distribution` | player_id, items, top_n | 3098 | For the radar/spider chart: one player's doubles partners, sorted by |
| `_response` | status_code, body_dict | 3145 | — |

#### `tournaments` — 3765 LOC
...
| `_validate_point_log` | point_log, score_a, score_b | 188 | Structural + count-consistency check for a live-scoring point_log, |
| `_is_valid_completed_game` | score_a, score_b, target | 209 | Same BWF-style rule as the standalone matches Lambda: win by 2 at |
| `_is_valid_manual_draft_game_score` | score_a, score_b | 238 | — |
| `_caller_claims` | event | 242 | Same pattern as matches lambda - see that file's comment for |
| `_is_super_admin` | claims | 251 | Ported from groups/index.py - identical logic, kept in sync by hand |
| `_authorize_tournament_organizer` | item, claims | 258 | Shared check for every manual-draft organizer-only write (set |
| `_authorize_pool_auction_viewer` | item, claims | 279 | Who may see pool assignments / auction budgets & bids for a |
| `create_tournament_enforced` | event | 300 | — |
| `handler` | event, context | 306 | — |
| `seeded_order` | players | 354 | Sort by current rating, descending. New players just use their |
| `pair_for_balance` | ordered_players | 364 | Given a skill-ordered list, pair strongest with weakest (snake |
| `create_tournament` | event | 379 | — |
| `build_round_robin` | entities | 521 | — |
| `build_knockout_round` | entities | 538 | — |
| `_bye_match` | entity | 573 | — |
| `handle_draft_route` | event | 600 | — |
| `_draft_get_tournament` | tournament_id | 671 | Shared load+validate for every route below: must exist and must be |
| `_draft_everyone` | item | 682 | Every player currently accounted for in this tournament's pool |
| `create_manual_draft_tournament` | event, claims | 691 | Creates the shell for a manual-mode tournament: leaders, pools, the |
| `set_leaders` | tournament_id, event, claims | 812 | — |
| `add_draft_player` | tournament_id, event, claims | 838 | Lets the organizer drop a player into the unassigned tray while |
| `remove_draft_player` | tournament_id, event, claims | 868 | The inverse of add_draft_player: drops someone out of this |
| `set_pool_assignment` | tournament_id, event, claims | 906 | Full replace of one pool's member list - the simplest, idempotent |
| `lock_pools` | tournament_id, event, claims | 966 | — |
| `_draft_decided_ids` | draft | 1027 | Every player_id that's no longer available to auction: already won |
| `_authorize_leader` | item, claims | 1038 | Caller must be one of THIS tournament's registered leaders (matched |
| `start_auction` | tournament_id, event, claims | 1049 | — |
| `open_lot` | tournament_id, event, claims | 1101 | — |
| `submit_bid` | tournament_id, event, claims | 1138 | — |
| `_maybe_freeze_squads` | item, draft | 1195 | Shared by close_lot and organizer_assign: once every leader's every |
| `close_lot` | tournament_id, event, claims | 1226 | — |
| `organizer_assign` | tournament_id, event, claims | 1258 | Lets the organizer record a winning bid and award a player entirely |
| `skip_lot` | tournament_id, event, claims | 1318 | — |
| `get_draft_state` | tournament_id, event, claims | 1340 | The polling endpoint - a small payload (no bid_history/full item) |
| `get_draft_sensitive_detail` | tournament_id, event, claims | 1366 | The privileged counterpart to the public GET /tournaments/{id}, |
| `build_tie` | squad_a_id, squad_b_id, matches_per_tie | 1404 | — |
| `build_tie_round_robin` | squad_ids, matches_per_tie | 1427 | — |
| `_bye_tie` | squad_id | 1435 | Mirrors _bye_match: auto-decided the instant it's created, no |
| `build_knockout_tie_round` | squad_ids, matches_per_tie | 1445 | Generalizes build_knockout_round: same power-of-2/byes-needed |
| `_update_tie_progress` | tie | 1470 | Recomputes wins_a/wins_b/point_diff_a/point_diff_b from the tie's |
| `_score_tie_match` | item, tie, match_index, score_a, score_b, ove | 1552 | Submits one individual match's score within a tie. Raises ValueError |
| `_cancel_tie_match` | tie, match_index | 1606 | Marks one match as administratively cancelled - can't be played |
| `_forfeit_tie_match` | tie, match_index, forfeited_by | 1629 | Marks one match as forfeited by one side (owner report, 2026-08-23: |
| `_find_tie` | item, tie_id | 1657 | A tie_id is a UUID unique across the whole tournament, so it can be |
| `_tie_side_leader_id` | item, side_id | 1674 | Resolves a tie's squad_a/squad_b value to the leader id who's |
| `_authorize_tie_scorer` | item, tie, claims | 1685 | Organizer, or one of THIS tie's own two squad leaders - matches the |
| `compute_squad_standings` | item, squad_ids | 1700 | Squad-level standings: sorted by (ties_won desc, aggregate point |
| `compute_projected_knockout` | item | 1741 | Read-time-only preview of the knockout matchup, computed from the |
| `compute_group_stage_projection` | item | 1793 | Real-separate-groups sibling of compute_projected_knockout (owner |
| `compute_squad_standings_by_parent` | item | 1847 | Cross-squad group mode sibling of compute_squad_standings: rolls |
| `compute_player_tournament_scores` | item | 1884 | A tournament-scoped, non-Elo per-player score/leaderboard - a |
| `rename_squad` | tournament_id, event, claims | 1964 | Squads get an auto-generated name ("Team <leader>") the instant the |
| `set_squad_pairs` | tournament_id, event, claims | 1998 | Cross-squad group mode only (owner request, 2026-08-21): before the |
| `move_squad_player` | tournament_id, event, claims | 2059 | Organizer-only roster rebalancing between two squads, before the |
| `_rebuild_entity_after_substitution` | entity, old_player_id, new_player_id, new_pla | 2105 | Swaps old_player_id for new_player_id inside a squad-pair/rep/ |
| `substitute_squad_player` | tournament_id, event, claims | 2130 | Organizer-only real substitution for a manual-draft squad: swaps a |
| `_build_group_stage` | item | 2250 | Shared schedule-building logic, used both by generate_schedule (the |
| `_fill_cross_squad_match_players` | item, ties | 2305 | Cross-squad group mode (owner request, 2026-08-21): a tie's two |
| `_build_cross_squad_group_stage` | item | 2334 | Cross-squad group mode (owner request, 2026-08-21): instead of |
| `generate_schedule` | tournament_id, event, claims | 2412 | — |
| `regenerate_schedule` | tournament_id, event, claims | 2447 | Organizer repair action: re-run schedule generation for a tournament |
| `pick_tie_player` | tournament_id, event, claims | 2508 | A leader nominates which of their own squad's members plays a given |
| `_generate_knockout_from_group_stage` | item | 2610 | — |
| `_inject_group_tiebreakers_if_needed` | item | 2629 | Real-separate-groups sibling of the legacy groups_then_knockout |
| `_advance_squads_to_knockout_from_groups` | item | 2673 | Real-separate-groups sibling of the legacy groups_then_knockout |
| `record_group_tie_score` | tournament_id, event, claims | 2725 | — |
| `_after_group_tie_resolved` | item | 2760 | Shared by every route that can make a group tie `decided` (score, |
| `cancel_group_tie_match` | tournament_id, event, claims | 2774 | Organizer-only: administratively cancels one group match that can |
| `forfeit_group_tie_match` | tournament_id, event, claims | 2810 | Organizer-only sibling of cancel_group_tie_match: one side didn't |
| `_advance_knockout_ties_if_round_complete` | item | 2845 | Mirrors record_knockout_score's round-advancement + third-place- |
| `record_knockout_tie_score` | tournament_id, event, claims | 2891 | — |
| `cancel_knockout_tie_match` | tournament_id, event, claims | 2930 | Organizer-only knockout/third-place sibling of cancel_group_tie_match |
| `forfeit_knockout_tie_match` | tournament_id, event, claims | 2966 | Organizer-only knockout/third-place sibling of forfeit_group_tie_match |
| `list_tournaments` | event | 3005 | — |
| `_redact_pool_auction_detail` | item | 3029 | GET /tournaments/{id} is unauthenticated - literally anyone browsing |
| `_hide_pool_auction_from_non_organizer` | item, claims | 3052 | pick_tie_player/record_group_tie_score/record_knockout_tie_score are |
| `get_tournament` | tournament_id | 3066 | — |
| `recompute_all_ratings` |  | 3107 | Elo is path-dependent - each match's rating change depends on the |
| `delete_tournament` | tournament_id, event | 3185 | Deletes this tournament AND every match record tagged with its |
| `compute_standings` | fixtures, entities | 3219 | — |
| `compute_all_standings` | item | 3251 | — |
| `_submit_game` | fixture, score_a, score_b, best_of, target, o | 3257 | Append one game's score to a fixture/match. Returns True if the match is now decided. |
| `record_group_score` | tournament_id, event | 3286 | — |
| `inject_tiebreakers_if_needed` | item | 3344 | Checks each subgroup for a genuine tie (same wins AND point_diff) at |
| `advance_to_knockout` | item | 3395 | — |
| `record_knockout_score` | tournament_id, event | 3420 | — |
| `compute_adaptive_k` | pairing_count | 3535 | Higher K for a fresh/novel doubles pairing (each match together is |
| `get_pairing_count` | team_ids | 3549 | How many prior doubles matches has this exact 2-player team played |
| `update_elo_and_log` | match_type, entity_a, entity_b, score_a, scor | 3567 | — |
| `substitute_player` | tournament_id, event | 3663 | Swap a player out of a team for all of that team's FUTURE (unplayed) |
| `_response` | status_code, body_dict | 3756 | — |
...
| Function | Args | Line | What it does |
|---|---|---|---|
| `_scan_all` | table | 99 | Full-table scan that follows LastEvaluatedKey - a bare .scan() returns |
| `_caller_claims` | event | 127 | Claims API Gateway's Cognito Authorizer attaches to the request. |
| `_is_super_admin` | claims | 135 | — |
| `_finance_role` | claims | 149 | — |
| `_finance_level` | claims | 165 | — |
| `_has_finance_access` | claims | 169 | View or better - the gate for reading finance at all. |
| `_default_group_id` |  | 174 | The group_id of the 'Club (default)' group that the pre-migration |
| `_group_for_request` | params, body | 190 | The group_id this finance op targets. Falls back to the default group |
| `_group_finance_level` | claims, group_id | 197 | A caller's finance level (0-3) FOR A SPECIFIC GROUP. |
| `_slot_key` | slot | 223 | Normalize a record's slot for bucketing/comparison: a missing/blank |
| `_member_assigned_slots` | pid, group | 230 | The set of slots (raw, already-normalized strings) a player is |
| `_view_scope_slots` | claims, group_id, level | 239 | Stage 4c: a plain 'view'-level grant only sees their own assigned |
| `_has_any_group_finance` | claims | 265 | True if the caller has finance access in ANY group (owner/admin, or a |
| `_effective_finance_role` | claims, group_id | 280 | The role name to REPORT to the frontend for button visibility: the |
| `finance_key_for_caller` | event | 305 | Hands the shared view key to any caller with finance access - global |
| `set_finance_access` | event | 322 | SuperAdmin sets a player's finance role directly. |
| `handler` | event, context | 346 | — |
| `_scan_type` | record_type, group_id | 468 | — |
| `_num` | v, default | 479 | — |
| `_clean` | record_type, data | 503 | — |
| `_resolve_name` | pid_cache, player_id | 515 | — |
| `_prev_period` | month, year | 531 | — |
| `_next_period` | month, year | 536 | — |
| `_member_relief` | settlement, memberships, ident, month, year,  | 541 | Relief a member gets in (month, year): the previous month's residual. |
| `_ensure_group_wide_membership_records` | group_id, month, year | 564 | Lazily create one slot=GROUP_SLOT membership record per distinct |
| `list_records` | record_type, params, group_id, scope_slots | 607 | — |
| `create_records` | record_type, body, group_id | 686 | — |
| `update_record` | record_type, record_id, body, group_id | 708 | — |
| `delete_record_enforced` | record_type, record_id, event | 782 | Triple-gated: SuperAdmin identity + FINANCE_VIEW_KEY + the existing |
| `delete_record` | record_type, record_id, body, group_id | 805 | — |
| `get_settings` |  | 822 | — |
| `put_settings` | body | 850 | — |
| `public_upi` |  | 879 | The pay card is shown to guests (they pay walk-in fees), so the UPI |
| `my_settlement` | claims, group_id | 887 | A single member's own dues in a group: for every (month, slot) where |
| `public_walkins` |  | 1037 | — |
| `_settlement_rows` | group_id | 1057 | Per (month, year, slot): the exact math from the Calculations sheet. |
| `summary` | group_id, scope_slots | 1291 | — |
| `_ledger_rows` | group_id | 1327 | Returns {(ident, month, year): row} for every member-month where the |
| `_compute_ledger_rows` | group_id, settlement, memberships | 1346 | The actual ledger computation, factored out of _ledger_rows so |
| `ledger` | group_id, params | 1494 | — |
| `save_ledger_entry` | body, group_id | 1506 | Upserts the one fact this whole ledger stores: what a member |
| `_parse_slot_window` | label | 1581 | Best-effort parse of a free-form slot label ('7AM-8AM', '19:00-20:00', |
| `_local_minutes_of_day` | iso_ts, offset_minutes | 1617 | Convert a stored ISO-8601 UTC match timestamp to local minute-of-day |
| `_minute_in_window` | minute, window, grace_minutes | 1637 | Whether `minute` (local minute-of-day) falls inside `window` (start, |
| `_timing_checks` | matches, group, offset_minutes, target_ym | 1650 | Best-effort, non-authoritative diagnostics only (see module note |
| `insights` | group_id | 1730 | Per-member monthly economics, ghosts, and walk-in conversion. |
| `_response` | status_code, body_dict | 2029 | — |
...
### Frontend (`frontend/js/app.js` — 13023 LOC, flat global script, ~509 functions)
...
- `bumpMatchesRev()` — L381
- `invalidateDataTabs()` — L406
- `isTabActive(tab)` — L407
- `ensureFresh(key, loader)` — L412
- `ensureOnce(key, loader)` — L418
- `loadStatsBundle()` — L424
- `makeCardsCollapsible(containerId)` — L454
- `setOpen(open)` — L469
- `makeStatsCollapsible()` — L474
- `makeFinanceCollapsible()` — L481
- `ensureProfileFresh()` — L486
- `loadActiveTabData()` — L495
- `myPlayerRecord()` — L506
- `iAmPrivate()` — L507
- `privateHiddenIds()` — L515
- `rankingHiddenIds()` — L528
- `renderPrivacyControl()` — L541
- `toggleMyPrivacy()` — L575
- `setPrivacyMode(value)` — L604
- `setPrivacyCooldown(value)` — L618
- `statsFetch(query)` — L632
- `populateAdminPrivacySelect()` — L638
- `adminSetPrivacy(makePrivate)` — L648
- `populateAdminRenameSelect()` — L675
- `adminRenamePlayer()` — L685
- `populateMergePlayerSelects()` — L715
- `mergePlayersAdmin()` — L730
- `seasonMedallion(rank, size)` — L776
- `seasonBadgeSvg(kind, rank, size)` — L790
- `loadPlayerSeasons(playerId)` — L806
- `loadSeasonsMeta()` — L833
- `loadSeasonBoard(seasonId)` — L855
- `renderSeasonAdmin()` — L886
- `saveSeasons(list, statusElId, okMsg)` — L895
- `addSeason()` — L906
- `deleteSeason(id)` — L920
- `setSeasonsEnabled(value)` — L924
- `setSeasonK(value)` — L935
- `loadPlayers()` — L943
- `loadGroups()` — L967
- `loadGroupMembers(groupId)` — L1032
- `opt(v, label)` — L1084
- `opt(v, label)` — L1108
- `nameOf(pid)` — L1121
- `applyGroupDefaultsToForm(prefix, settings)` — L1156
- `setIfPresent(suffix, value)` — L1158
- `renderAddPlayersChecklist()` — L1169
- `removePlayerFromGroup(groupId, playerId)` — L1182
- `populateTeamSelects()` — L1217
- `refreshTeamSelectOptions()` — L1256

**Segmented controls (match type / points-to-win)**  (from L1273)
- `syncTeamSelectValues()` — L1274
- `handleTeamSelectChange(changedId)` — L1283
- `applyMatchTypeVisibility()` — L1297
- `nwSyncSegFromSelect(selectId, segId)` — L1314
- `updateMatchGroupCache()` — L1334

**Live point-by-point scoring**  (from L1353)
- `randomizeTeams(showAlertOnFail)` — L1355
- `isGameOver(a, b, target)` — L1393
- `updateLiveScoreDisplay()` — L1401

**Split-screen live scoring**  (from L1434)
- `getTeamDisplayName(selectId)` — L1471
- `getSplitTeamNames()` — L1477
- `updateSplitScreenScores(a, b, over)` — L1492
- `openSplitScreenGeneric(config)` — L1501
- `closeSplitScreen()` — L1510
- `openSplitScreen()` — L1516
- `openTournamentSplitScreen(matchKey, target, nameA, nameB, finishFn)` — L1541

**Groups**  (from L1739)
- `prefillEditForm()` — L1741

**Matches (record/list/game-log)**  (from L1868)
- `myGroups()` — L1933
- `visibleGroupsForFilter()` — L1944
- `defaultMatchGroup()` — L1952

**Voice match entry**  (from L1957)
- `defaultRegisterGroup()` — L1983
- `applyVoiceVisibility()` — L2001
- `nwPhon(s)` — L2008
- `nwLev(a, b)` — L2019
- `nwScorePlayer(token, p)` — L2026
- `nwMatchPlayerToken(tokenRaw)` — L2045
- `nwWordsToNums(t)` — L2060
- `nwParseMatchTranscript(raw)` — L2068
- `nwApplyParsedToForm(p)` — L2100
- `set(id, entry)` — L2104
- `nwVoicePreviewHtml(p)` — L2114
- `nwVoiceMatchInit()` — L2126
- `stopListening()` — L2155

**Team pairing preview**  (from L2205)
- `nwSeeded(p)` — L2244
- `nwShuffle(a)` — L2245
- `nwPairingRefreshList()` — L2247
- `nwPairingUpdateCount()` — L2258
- `nwPairingRender()` — L2263
- `nwPairingInit()` — L2295

**Quick record: tap mode + Sessions + shared queue (server-synced, polled)**  (from L2301)
- `postMatchPayload(payload)` — L2352
- `nwSetRecordMode(mode)` — L2362
- `nwTapRosterPool()` — L2407
- `nwLoadGroupSessions()` — L2415
- `nwRenderSessionBar()` — L2435
- `nwRenderOnCourtActions()` — L2491
- `nwOpenSessionAddPanel()` — L2577
- `nwSetSessionAddMode(mode)` — L2601
- `nwUpdateSessionAddCount()` — L2612
- `nwSessionAddMember(body)` — L2621
- `nwSessionRemoveMember(playerId)` — L2670
- `nwTapSlotsPerTeam()` — L2692
- `nwTapRefreshAvatarGrid()` — L2696
- `nwCloseGuestPanel()` — L2802
- `nwSetGuestAddMode(mode)` — L2806
- `nwTapToggleAvatar(playerId)` — L2860
- `nwTapPlayerName(id)` — L2869
- `nwTapRenderTeams()` — L2874
- `nwTapPointsToWin()` — L2911
- `nwTapFinalScore()` — L2912
- `nwTapRenderQuickScore()` — L2916
- `nwTapRenderManual()` — L2948
- `nwStackVoiceNote(said)` — L3074
- `nwQueueAddedByLabel(item)` — L3137
- `nwQueuePushItem(fields)` — L3143
- `nwRenderQueue()` — L3163
- `groupName(gid)` — L3169
- `stopQueuePolling()` — L3208
- `nwMaybeStartQueuePolling()` — L3212
- `nwQueueFetchTick(groupId)` — L3225
- `nwQueueEditItem(id)` — L3242
- `nwQueueRemoveItem(id)` — L3281
- `nameFor(pid)` — L3386

**Unsaved-match safety net**  (from L3424)
- `showMatchOutcome(ok, message)` — L3456
- `savePendingMatch(payload, meta)` — L3475
- `loadPendingMatch()` — L3479
- `clearPendingMatch()` — L3482
- `handleSessionExpired()` — L3489

**Game log & CSV export**  (from L3510)
- `ensureRestoreHost()` — L3512
- `offerPendingMatchRestore()` — L3521
- `loadGameLog()` — L3556
- `gameLogGoto(p)` — L3606
- `renderGameLog()` — L3608
- `matchPermissions(m)` — L3664
- `matchGroupLabel(m)` — L3686
- `canActOnMatchDirectly(m)` — L3704
- `requestMatchChange(matchId, type, label, groupId, extra)` — L3711
- `editMatch(matchId, groupId)` — L3735
- `opts(sel)` — L3744
- `pickers(team, prefix)` — L3746
- `close()` — L3767
- `editMatchScore(matchId, currentScoreA, currentScoreB, e)` — L3792
- `deleteMatch(matchId, encLabel, groupId)` — L3828
- `downloadCSV(filename, rows)` — L3860
- `loadRankings()` — L3894
- `gp(p)` — L3920
- `fetchRatingHistory(playerId)` — L3965
- `loadVisiblePlayers(opts = {})` — L3976

**Profile card customization**  (from L4007)
- `resolveBannerId(id)` — L4114
- `bgCss(id, url)` — L4188
- `updatePageBackground()` — L4193
- `applyPageBackground(player)` — L4204
- `renderProfileCardBanner(player)` — L4211
- `toggleHeaderMenu()` — L4255
- `openSettingsModal()` — L4269
- `loadFinanceAccessList()` — L4283
- `opt(v, label)` — L4301
- `setGroupFinanceRole(groupId, playerId, role)` — L4316
- `setGroupMemberRole(groupId, playerId, role, wasRole, isSelf)` — L4332
- `setFinanceRole(playerId, role)` — L4348
- `closeSettingsModal()` — L4359
- `renderSettingsPickers(player)` — L4363
- `swatch(field, id, css, selected)` — L4372
- `submitClaimRequest()` — L4401
- `checkApprovalStatus()` — L4426
- `recomputeNow()` — L4439
- `loadAppSettings()` — L4454

**Quests**  (from L4484)
- `setXpPublic(value)` — L4486
- `setVoiceEnabled(value)` — L4501
- `setInstantCreate(value)` — L4516
- `loadQuests()` — L4529
- `_renderQuestRow(q)` — L4538
- `_hdr(t)` — L4562
- `claimQuest(questId)` — L4570
- `loadQuestsAdmin()` — L4586

**Achievements**  (from L4596)
- `saveQuest()` — L4607
- `deleteQuest(questId)` — L4629
- `badgeSvg(tier, glyph)` — L4656
- `loadMyAchievements()` — L4680
- `claimAchievement(achievementId)` — L4715
- `loadAchievementsAdmin()` — L4742
- `editAchievement(achievementId)` — L4800
- `cancelAchievementEdit()` — L4817
- `saveAchievement()` — L4832
- `deleteAchievement(achievementId)` — L4859
- `revokeAchievementClaim(achievementId)` — L4874

**Store & events admin**  (from L4915)
- `seedStarterAchievements()` — L4918
- `loadStore()` — L4960
- `catOf(i)` — L4985
- `cardHtml(i)` — L4990
- `buyStoreItem(itemId)` — L5022
- `onStoreImagePick(input)` — L5036
- `loadStoreAdmin()` — L5044
- `onStoreTypeChange()` — L5072
- `onStoreEffectChange()` — L5084
- `uploadStoreImage(file)` — L5100
- `saveStoreItem()` — L5117
- `deleteStoreItem(itemId)` — L5167
- `loadEventsAdmin()` — L5178
- `editEvent(e)` — L5200
- `saveEvent()` — L5208
- `deleteEvent(eventId)` — L5231
- `refreshEventBanner()` — L5243
- `loadClaimAudit()` — L5260
- `relinkAccount(usernameEnc, presetPlayerId)` — L5327
- `unlinkAccount(usernameEnc)` — L5335
- `unlinkAndStrip(usernameEnc, playerId)` — L5340
- `_claimAuditAction(bodyObj)` — L5345
- `loadUnconfirmedUsers()` — L5356
- `deleteUnconfirmedUser(username, email)` — L5383
- `loadClaimRequests()` — L5396

**Image uploads**  (from L5450)
- `decideClaimRequest(requestId, action, requestType)` — L5451
- `escapeHtml(s)` — L5489
- `resizeImage(file, kind)` — L5506
- `isAnimatedImage(file)` — L5541
- `uploadCardImage(kind, fileInput)` — L5553
- `imageSrc(key)` — L5608
- `loadStoreCatalogOnce()` — L5614
- `renderStoreCosmeticStrip(kind, player)` — L5624
- `renderUploadStrip(kind, player)` — L5648
- `vsPlayerVisual(pid, snapshot)` — L5680
- `vsAvatarHtml(v, isWinner)` — L5696
- `teamBanner(side)` — L5713
- `gameScore(game, side)` — L5724
- `renderVsCard(idsA, idsB, opts = {})` — L5730
- `won(side)` — L5735
- `vsSideIds(side)` — L5760
- `setMyCardField(field, value)` — L5770

**Profile bundle / cards / charts**  (from L5794)
- `loadProfileBundle(playerId)` — L5838
- `renderTieredCard(icon, name, unit, tiers, currentValue)` — L5953
- `renderBinaryCard(icon, name, desc, achieved, detail)` — L5981
- `resetRatingZoom()` — L6053
- `loadProfileRatingChart(playerId)` — L6059
- `loadProfilePartnershipsAndRadar(playerId)` — L6145
- `loadProfileHeadToHead(playerId)` — L6194
- `loadProfileWithPartner(playerId)` — L6218
- `partnerGamesGoto(p)` — L6250
- `renderPartnerGames()` — L6252
- `skeletonHTML(lines = 3)` — L6285
- `showProfileSkeletons()` — L6292
- `renderXpPanel(player)` — L6305
- `xpForLevel(n)` — L6313
- `updateHeaderCoins()` — L6339
- `loadProfile()` — L6351
- `refreshProfile()` — L6381
- `refreshProfileIfShowing(affectedPlayerIds)` — L6398
- `renderPartnerRadar(data, highlightTournament, svgId = 'rada)` — L6419
- `loadHistory()` — L6475
- `renderHistory(data)` — L6493
- `loadBadges()` — L6554
- `renderBadges(data)` — L6572
- `loadDiversity()` — L6605
- `renderDiversity(data)` — L6623
- `playerLabelById(playerId, fallbackName)` — L6644
- `playerLabelsById(playerIds, fallbackNames)` — L6648
- `loadHallOfFame()` — L6654
- `renderHallOfFame(data)` — L6676

**UPI payment card**  (from L6750)
- `loadAttendance()` — L6760
- `renderAttendance(data)` — L6779
- `refreshUpiCard()` — L6798

**Finance tab (view-key + role gated)**  (from L6808)
- `renderUpiCard()` — L6810
- `imageServiceFallback()` — L6832
- `xpVisible()` — L6860
- `applyFinanceRoleVisibility()` — L6866
- `refreshFinanceRoleForGroup()` — L6897
- `finQS(extra)` — L6910
- `financeBaseUrl()` — L6921
- `finPost(path, method, bodyObj)` — L6925
- `populateFinanceSlots(group)` — L6949
- `_rememberedFinance(key)` — L6979
- `_rememberFinance(key, val)` — L6983
- `restoreFinanceMonth()` — L6989
- `populateFinanceGroups()` — L6998
- `reloadFinanceForGroup()` — L7026
- `tryAutoFinanceUnlock()` — L7031
- `myFinanceGroups()` — L7061
- `populateMyDuesGroups()` — L7066
- `loadMyDues(groupId)` — L7084
- `manageGroupSlots(groupId)` — L7134
- `assignSlotMembers(groupId, slotEnc)` — L7152
- `transferGroupOwnership(groupId)` — L7180
- `setGroupPayee(groupId)` — L7199
- `requestFinanceAccess()` — L7222
- `financeUnlock()` — L7239
- `updateFinanceScopeNote(scopedTo)` — L7294
- `loadFinanceSummary()` — L7304
- `loadFinanceExpenses()` — L7344
- `resetExpenseEdit()` — L7382
- `addFinanceExpense()` — L7389
- `loadFinanceMembers()` — L7408
- `markMembersDirty()` — L7496
- `recalcMembers()` — L7503
- `loadFinanceLedger()` — L7521
- `saveLedgerEntry(card, amount)` — L7589
- `renderBulkRosterList()` — L7622
- `bulkAddFromRoster()` — L7636
- `copyPreviousMonthMembers()` — L7651
- `addFinanceMember()` — L7687
- `resetWalkinEdit()` — L7713
- `suggestWalkinSessions()` — L7732
- `loadFinanceWalkins()` — L7764
- `addFinanceWalkin()` — L7816
- `loadFinanceInsights()` — L7849
- `copyDuesForWhatsApp()` — L7863
- `pad(s, w)` — L7880
- `padL(s, w)` — L7881
- `line(n, o, r, p)` — L7882
- `done()` — L7892
- `fallbackCopy(text, cb)` — L7898
- `copyInsightsTableAsImage()` — L7927
- `renderInsights()` — L8059

**Match review & reorder (SuperAdmin)**  (from L8160)
- `saveFinanceSettings()` — L8194
- `loadPublicWalkins()` — L8240
- `loadReviewDay()` — L8337
- `reviewOrderChanged()` — L8381
- `renderReviewList()` — L8387
- `applyReviewOrder()` — L8448
- `updateAuthUI()` — L8476
- `hiddenNow(id, btn)` — L8504
- `refreshMySession(statusElId)` — L8589
- `setStatus(msg)` — L8590
- `openAchievementsModal()` — L8618
- `closeAchievementsModal()` — L8627
- `openAuthModal()` — L8628
- `closeAuthModal()` — L8629
- `showAuthView(view)` — L8630
- `setAuthSession(session, user, opts = {})` — L8638
- `closeCompleteProfileModal()` — L8665
- `openCompleteProfileModal()` — L8666
- `showCompleteProfileMode(mode, preselectPlayerId)` — L8681
- `populateClaimPicker(preselectPlayerId)` — L8689
- `submitClaimProfile()` — L8713
- `closeCompleteProfileModal()` — L8753
- `sanitizeNickname(raw)` — L8759
- `editDistance(a, b)` — L8764
- `checkForExistingPlayer(name, typedNickname, statusEl)` — L8786
- `submitCompleteProfile()` — L8841
- `finishRequestAndSignOut(message)` — L8917
- `doLogin()` — L8923
- `doNewPassword()` — L8976
- `doSignup()` — L8987

**Tournaments**  (from L9016)
- `doConfirmSignup()` — L9032
- `doResendConfirmCode()` — L9063
- `doForgotPassword()` — L9074
- `doConfirmForgotPassword()` — L9089
- `doLogout()` — L9101
- `restoreSession()` — L9151
- `restoreTabFromHash()` — L9195
- `addManualTeamRow()` — L9270
- `collectManualTeams()` — L9306
- `loadTournamentGroupOptions()` — L9319
- `loadTournamentParticipantsChecklist()` — L9328
- `updateParticipantsCount()` — L9358
- `collectTournamentParticipants()` — L9370
- `loadTournamentsList()` — L9374
- `submitTournamentCreation(payload)` — L9381
- `submitManualDraftCreation(group_id, name)` — L9407
- `draftPlayerName(pid)` — L9455
- `draftEveryone(t)` — L9460
- `renderManualDraftTournament(t)` — L9466
- `fetchTournamentDetail(tournamentId)` — L9565
- `fetchAndRenderTournamentDetail(tournamentId)` — L9581
- `stopSchedulePolling()` — L9612
- `startSchedulePolling(tournamentId)` — L9617
- `isSchedulePollingActiveFor(tournamentId)` — L9627
- `schedulePollTick(tournamentId)` — L9629
- `renderDraftLeaderPicker(t)` — L9660
- `saveDraftLeaders(tournamentId)` — L9680
- `renderDraftPoolBoard(t)` — L9690
- `chip(pid)` — L9695
- `draftChipTapped(pid, ev)` — L9742
- `draftPoolColumnTapped(tournamentId, poolName)` — L9749
- `draftChipDragStart(ev, pid)` — L9756
- `draftPoolDragOver(ev)` — L9761
- `draftPoolDrop(ev, tournamentId, poolName)` — L9766
- `moveDraftPlayerToPool(tournamentId, poolName, playerId)` — L9775
- `putDraftPool(tournamentId, poolName, playerIds)` — L9795
- `addNewDraftPlayer(tournamentId, groupId)` — L9805
- `removeDraftPlayer(tournamentId, playerId)` — L9831
- `lockDraftPools(tournamentId)` — L9844
- `stopDraftPolling()` — L9874
- `startDraftPolling(tournamentId)` — L9879
- `isDraftPollingActiveFor(tournamentId)` — L9892
- `pollDraftStateTick(tournamentId)` — L9894
- `draftDecidedIds(draft)` — L9905
- `renderDraftStartAuctionPanel(t)` — L9914
- `startDraftAuction(tournamentId)` — L9923
- `renderDraftAuctionRoom(t)` — L9933
- `draftAssignEligibleLeaders(t, pool)` — L9948
- `draftAssignLeaderOptionsHtml(t, pool)` — L9958
- `updateDraftAssignLeaderOptions()` — L9964
- `renderDraftOrganizerAssignPanel(t)` — L9973
- `organizerAssignPlayer(tournamentId)` — L9998
- `renderDraftLiveStatusHtml(tournamentId, draftLike)` — L10019
- `updateDraftLiveStatus(tournamentId, draftLike)` — L10054
- `renderDraftQueuePicker(t)` — L10067
- `openDraftLot(tournamentId, playerId)` — L10094
- `closeDraftLot(tournamentId)` — L10102
- `skipDraftLot(tournamentId)` — L10112
- `renderDraftBidBox()` — L10122
- `draftBidBump(delta)` — L10135
- `submitDraftBid(tournamentId)` — L10142
- `renderDraftSquadsReview(t)` — L10171
- `renderSetSquadPairsPanel(t)` — L10191
- `generateCrossSquadGroups(tournamentId, status)` — L10254
- `saveSquadPairs(tournamentId, squadId, numGroups, slotsP)` — L10271
- `generateDraftSchedule(tournamentId)` — L10292
- `renderSquadRosterEditPanel(t, allowMove)` — L10313
- `renameSquadPrompt(tournamentId, squadId)` — L10379
- `moveSquadPlayer(tournamentId)` — L10396
- `toggleSquadSubNewPlayerFields(useNew)` — L10410
- `substituteSquadPlayer(tournamentId)` — L10427
- `draftSquadName(t, squadId)` — L10479
- `toggleDraftGroupOpen(name, detailsEl)` — L10501
- `toggleDraftSquadSection(key, detailsEl)` — L10514
- `renderDraftScheduleView(t)` — L10518
- `renderProjectedKnockout(t)` — L10615
- `renderRegenerateScheduleGroupPanel(t)` — L10637
- `regenerateDraftSchedule(tournamentId)` — L10661
- `renderSquadStandingsTable(standings, projection)` — L10688
- `computeLeaderboardRows(stats, t)` — L10727
- `tallyPair(side)` — L10764
- `squadSideField(tie, sid)` — L10826
- `decidingPairKey(tie, sid)` — L10827
- `podiumRank(row)` — L10885
- `renderPlayerTournamentStatsTable(stats, t)` — L10901
- `rowHtml(row)` — L10909
- `renderTieSection(title, ties, t, stageKind)` — L10954
- `renderTieCard(tie, t, stageKind)` — L10961
- `renderTieMatchRow(tie, m, idx, t, stageKind, iLeadA, iLead)` — L10993
- `draftTieMatchAdminControlsHtml(tournamentId, tieId, idx, stageKind, sid)` — L11179
- `cancelDraftTieMatch(tournamentId, tieId, matchIndex, stageKi)` — L11187
- `forfeitDraftTieMatch(tournamentId, tieId, matchIndex, stageKi)` — L11198
- `draftPlayerPickerHtml(tournamentId, tieId, matchIndex, members)` — L11209
- `opts(selected)` — L11215
- `pickTiePlayer(tournamentId, tieId, matchIndex, playerI)` — L11235
- `pickTiePlayerPair(tournamentId, tieId, matchIndex, squadId)` — L11247
- `submitDraftTieScore(tournamentId, tieId, matchIndex, stageKi)` — L11263
- `submitDraftTieScoreDirect(tournamentId, tieId, matchIndex, stageKi)` — L11280
- `collectAllEntities(t)` — L11446
- `getAllTeamEntities(t)` — L11462
- `renderTeamCompositionBars(t, containerId)` — L11480
- `populateSubstitutionSection(t)` — L11515
- `updateSubOldPlayerOptions()` — L11526
- `formatGames(games)` — L11615
- `applyTournamentViewMode()` — L11622
- `matchTotals(match)` — L11638
- `truncateBracketName(name, maxChars = 22)` — L11646
- `renderBracketView(t)` — L11651
- `renderDraftBracketGroupsPanel(t)` — L11779
- `renderDraftBracketView(t)` — L11815
- `renderTournament(t)` — L11946
- `generateTournamentRecap(t)` — L12121
- `downloadTournamentImage()` — L12153
- `loadImg(src)` — L12180
- `sideVisuals(side)` — L12190
- `drawCard(x, y, w, match, isFinal)` — L12197
- `drawAvatars(ctx, x, y, side, isWinner)` — L12243
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L12262
- `roundRect(ctx, x, y, w, h, r)` — L12290
- `downloadDraftShareImage()` — L12306
- `loadImg(src)` — L12313
- `sideAvatars(squadId)` — L12362
- `drawSide(side, sx, sy, isWinner)` — L12443
- `downloadDraftLeaderboardImage()` — L12524
- `loadImg(src)` — L12533
- `presetKeyFor(bannerCss)` — L12542

**Live scoring inside tournaments**  (from L12563)
- `copyTournamentRecap()` — L12652
- `item_has_third_place(t)` — L12663
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L12667
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L12673
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L12710
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L12716
- `submitThirdPlaceScore(tournamentId)` — L12743
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L12749
- `getTournamentLiveLog(matchKey)` — L12780
- `tournamentLivePoint(matchKey, side, target)` — L12785
- `tournamentUndoPoint(matchKey, target)` — L12794
- `updateTournamentLiveDisplay(matchKey, target)` — L12800
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L12818
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L12832
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L12841
- `finishDraftTieLiveGame(matchKey, tournamentId, tieId, matchInde)` — L12859
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L12868
- `activateTab(tabName)` — L12892
- `jumpToRecordMatch()` — L12993
- `applyTheme(theme)` — L12999
