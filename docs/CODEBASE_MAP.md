...
### Frontend (`frontend/js/app.js` — 7310 LOC, flat global script, ~296 functions)
...
- `markMembersDirty()` — L4917
- `recalcMembers()` — L4924
- `renderBulkRosterList()` — L4936
- `bulkAddFromRoster()` — L4950
- `copyPreviousMonthMembers()` — L4965
- `addFinanceMember()` — L5001
- `loadFinanceWalkins()` — L5022
- `addFinanceWalkin()` — L5049
- `loadFinanceInsights()` — L5087
- `copyDuesForWhatsApp()` — L5101
- `pad(s, w)` — L5118
- `padL(s, w)` — L5119
- `line(n, o, r, p)` — L5120
- `done()` — L5130
- `fallbackCopy(text, cb)` — L5136
- `renderInsights()` — L5145
- `saveFinanceSettings()` — L5218
- `loadPublicWalkins()` — L5255
- `loadReviewDay()` — L5334

**Tournaments**  (from L5371)
- `reviewOrderChanged()` — L5378
- `renderReviewList()` — L5384
- `applyReviewOrder()` — L5433
- `updateAuthUI()` — L5460
- `hiddenNow(id, btn)` — L5477
- `openAuthModal()` — L5547
- `closeAuthModal()` — L5548
- `showAuthView(view)` — L5549
- `setAuthSession(session, user, opts = {})` — L5557
- `openCompleteProfileModal()` — L5574
- `showCompleteProfileMode(mode, preselectPlayerId)` — L5589
- `populateClaimPicker(preselectPlayerId)` — L5597
- `submitClaimProfile()` — L5621
- `closeCompleteProfileModal()` — L5661
- `sanitizeNickname(raw)` — L5667
- `editDistance(a, b)` — L5672
- `checkForExistingPlayer(name, typedNickname, statusEl)` — L5694
- `submitCompleteProfile()` — L5749
- `finishRequestAndSignOut(message)` — L5825
- `doLogin()` — L5831
- `doNewPassword()` — L5884
- `doSignup()` — L5895
- `doConfirmSignup()` — L5912
- `doResendConfirmCode()` — L5943
- `doForgotPassword()` — L5954
- `doConfirmForgotPassword()` — L5969
- `doLogout()` — L5981
- `restoreSession()` — L6025
- `restoreTabFromHash()` — L6059
- `addManualTeamRow()` — L6125
- `collectManualTeams()` — L6161
- `loadTournamentGroupOptions()` — L6174
- `loadTournamentParticipantsChecklist()` — L6183
- `updateParticipantsCount()` — L6213
- `collectTournamentParticipants()` — L6225
- `loadTournamentsList()` — L6229
- `submitTournamentCreation(payload)` — L6236
- `collectAllEntities(t)` — L6395
- `getAllTeamEntities(t)` — L6411

**Live scoring inside tournaments**  (from L6421)
- `renderTeamCompositionBars(t, containerId)` — L6429
- `populateSubstitutionSection(t)` — L6464
- `updateSubOldPlayerOptions()` — L6475
- `formatGames(games)` — L6564
- `applyTournamentViewMode()` — L6571
- `matchTotals(match)` — L6577
- `truncateBracketName(name, maxChars = 22)` — L6585
- `renderBracketView(t)` — L6590
- `renderTournament(t)` — L6706
- `generateTournamentRecap(t)` — L6880
- `downloadTournamentImage()` — L6912
- `loadImg(src)` — L6939
- `sideVisuals(side)` — L6949
- `drawCard(x, y, w, match, isFinal)` — L6956
- `drawAvatars(ctx, x, y, side, isWinner)` — L7002
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L7021
- `roundRect(ctx, x, y, w, h, r)` — L7049
- `copyTournamentRecap()` — L7059
- `item_has_third_place(t)` — L7070
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L7074
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L7080
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L7099
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L7105
- `submitThirdPlaceScore(tournamentId)` — L7124
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L7130
- `getTournamentLiveLog(matchKey)` — L7153
- `tournamentLivePoint(matchKey, side, target)` — L7158
- `tournamentUndoPoint(matchKey, target)` — L7167
- `updateTournamentLiveDisplay(matchKey, target)` — L7173
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L7191
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L7200
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L7209
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L7218
- `applyTheme(theme)` — L7291
