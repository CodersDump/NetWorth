...
### Frontend (`frontend/js/app.js` — 10678 LOC, flat global script, ~448 functions)
...
- `tallyPair(side)` — L8733
- `squadSideField(tie, sid)` — L8766
- `decidingPairKey(tie, sid)` — L8767
- `rowHtml(row)` — L8818
- `renderTieSection(title, ties, t, stageKind)` — L8863
- `renderTieCard(tie, t, stageKind)` — L8870
- `renderTieMatchRow(tie, m, idx, t, stageKind, iLeadA, iLead)` — L8889
- `draftPlayerPickerHtml(tournamentId, tieId, matchIndex, members)` — L9027
- `opts(selected)` — L9033
- `pickTiePlayer(tournamentId, tieId, matchIndex, playerI)` — L9053
- `pickTiePlayerPair(tournamentId, tieId, matchIndex, squadId)` — L9065
- `submitDraftTieScore(tournamentId, tieId, matchIndex, stageKi)` — L9081
- `submitDraftTieScoreDirect(tournamentId, tieId, matchIndex, stageKi)` — L9098
- `collectAllEntities(t)` — L9264
- `getAllTeamEntities(t)` — L9280
- `renderTeamCompositionBars(t, containerId)` — L9298
- `populateSubstitutionSection(t)` — L9333
- `updateSubOldPlayerOptions()` — L9344
- `formatGames(games)` — L9433
- `applyTournamentViewMode()` — L9440
- `matchTotals(match)` — L9456
- `truncateBracketName(name, maxChars = 22)` — L9464
- `renderBracketView(t)` — L9469
- `renderDraftBracketGroupsPanel(t)` — L9597
- `renderDraftBracketView(t)` — L9633
- `renderTournament(t)` — L9764
- `generateTournamentRecap(t)` — L9939
- `downloadTournamentImage()` — L9971
- `loadImg(src)` — L9998
- `sideVisuals(side)` — L10008
- `drawCard(x, y, w, match, isFinal)` — L10015
- `drawAvatars(ctx, x, y, side, isWinner)` — L10061
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L10080
- `roundRect(ctx, x, y, w, h, r)` — L10108
- `downloadDraftShareImage()` — L10124
- `loadImg(src)` — L10131
- `sideAvatars(squadId)` — L10180
- `drawSide(side, sx, sy, isWinner)` — L10261
- `copyTournamentRecap()` — L10317
- `item_has_third_place(t)` — L10328
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L10332
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L10338
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L10375
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L10381
- `submitThirdPlaceScore(tournamentId)` — L10408
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L10414
- `getTournamentLiveLog(matchKey)` — L10445
- `tournamentLivePoint(matchKey, side, target)` — L10450
- `tournamentUndoPoint(matchKey, target)` — L10459
- `updateTournamentLiveDisplay(matchKey, target)` — L10465
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L10483
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L10497
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L10506
- `finishDraftTieLiveGame(matchKey, tournamentId, tieId, matchInde)` — L10524
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L10533
- `activateTab(tabName)` — L10557
- `jumpToRecordMatch()` — L10653
- `applyTheme(theme)` — L10659
