...
### Frontend (`frontend/js/app.js` — 6571 LOC, single IIFE, ~257 functions)
...
- `doNewPassword()` — L5171
- `doSignup()` — L5182
- `doConfirmSignup()` — L5199
- `doForgotPassword()` — L5227
- `doConfirmForgotPassword()` — L5242
- `doLogout()` — L5254
- `restoreSession()` — L5298
- `restoreTabFromHash()` — L5331

**Tournaments**  (from L5371)
- `addManualTeamRow()` — L5397
- `collectManualTeams()` — L5433
- `loadTournamentGroupOptions()` — L5446
- `loadTournamentParticipantsChecklist()` — L5455
- `updateParticipantsCount()` — L5485
- `collectTournamentParticipants()` — L5497
- `loadTournamentsList()` — L5501
- `submitTournamentCreation(payload)` — L5508
- `collectAllEntities(t)` — L5667
- `getAllTeamEntities(t)` — L5683
- `renderTeamCompositionBars(t, containerId)` — L5701
- `populateSubstitutionSection(t)` — L5736
- `updateSubOldPlayerOptions()` — L5747
- `formatGames(games)` — L5836
- `applyTournamentViewMode()` — L5843
- `matchTotals(match)` — L5849
- `truncateBracketName(name, maxChars = 22)` — L5857
- `renderBracketView(t)` — L5862
- `renderTournament(t)` — L5978
- `generateTournamentRecap(t)` — L6152
- `downloadTournamentImage()` — L6184
- `loadImg(src)` — L6211
- `sideVisuals(side)` — L6221
- `drawCard(x, y, w, match, isFinal)` — L6228
- `drawAvatars(ctx, x, y, side, isWinner)` — L6274
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L6293
- `roundRect(ctx, x, y, w, h, r)` — L6321
- `copyTournamentRecap()` — L6331
- `item_has_third_place(t)` — L6342
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L6346
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L6352
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L6371
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L6377
- `submitThirdPlaceScore(tournamentId)` — L6396
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L6402

**Live scoring inside tournaments**  (from L6421)
- `getTournamentLiveLog(matchKey)` — L6425
- `tournamentLivePoint(matchKey, side, target)` — L6430
- `tournamentUndoPoint(matchKey, target)` — L6439
- `updateTournamentLiveDisplay(matchKey, target)` — L6445
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L6463
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L6472
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L6481
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L6490
- `applyTheme(theme)` — L6552
