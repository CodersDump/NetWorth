...
### Frontend (`frontend/js/app.js` — 7190 LOC, flat global script, ~287 functions)
...
- `loadProfilePartnershipsAndRadar(playerId)` — L3640
- `loadProfileHeadToHead(playerId)` — L3689
- `loadProfileWithPartner(playerId)` — L3713
- `partnerGamesGoto(p)` — L3745
- `renderPartnerGames()` — L3747
- `skeletonHTML(lines = 3)` — L3780
- `showProfileSkeletons()` — L3787
- `renderXpPanel(player)` — L3800
- `xpForLevel(n)` — L3808
- `updateHeaderCoins()` — L3834
- `loadProfile()` — L3846
- `refreshProfile()` — L3871
- `refreshProfileIfShowing(affectedPlayerIds)` — L3888
- `renderPartnerRadar(data, highlightTournament, svgId = 'rada)` — L3909

**UPI payment card**  (from L3941)
- `loadHistory()` — L3965
- `renderHistory(data)` — L3983

**Finance tab (view-key + role gated)**  (from L3999)
- `loadBadges()` — L4044
- `renderBadges(data)` — L4062
- `loadDiversity()` — L4095
- `renderDiversity(data)` — L4113
- `playerLabelById(playerId, fallbackName)` — L4134
- `playerLabelsById(playerIds, fallbackNames)` — L4138
- `loadHallOfFame()` — L4144
- `renderHallOfFame(data)` — L4166
- `loadAttendance()` — L4250
- `renderAttendance(data)` — L4269
- `refreshUpiCard()` — L4288
- `renderUpiCard()` — L4300
- `imageServiceFallback()` — L4322
- `xpVisible()` — L4350
- `applyFinanceRoleVisibility()` — L4356
- `finQS(extra)` — L4377
- `financeBaseUrl()` — L4388
- `finPost(path, method, bodyObj)` — L4392
- `populateFinanceGroups()` — L4413
- `reloadFinanceForGroup()` — L4440
- `tryAutoFinanceUnlock()` — L4445
- `myFinanceGroups()` — L4470
- `populateMyDuesGroups()` — L4475
- `loadMyDues(groupId)` — L4493
- `manageGroupSlots(groupId)` — L4538
- `assignSlotMembers(groupId, slotEnc)` — L4556
- `transferGroupOwnership(groupId)` — L4584
- `setGroupPayee(groupId)` — L4603
- `requestFinanceAccess()` — L4626
- `financeUnlock()` — L4643

**Match review & reorder (SuperAdmin)**  (from L4649)
- `loadFinanceSummary()` — L4681
- `loadFinanceExpenses()` — L4702
- `resetExpenseEdit()` — L4740
- `addFinanceExpense()` — L4747
- `loadFinanceMembers()` — L4766

**Auth UI (Cognito login/signup/session)**  (from L4780)
- `scheduleMemberReload()` — L4855
- `renderBulkRosterList()` — L4863
- `bulkAddFromRoster()` — L4877
- `copyPreviousMonthMembers()` — L4892
- `addFinanceMember()` — L4928
- `loadFinanceWalkins()` — L4949
- `addFinanceWalkin()` — L4976
- `loadFinanceInsights()` — L5014
- `renderInsights()` — L5028
- `saveFinanceSettings()` — L5100
- `loadPublicWalkins()` — L5137
- `loadReviewDay()` — L5214
- `reviewOrderChanged()` — L5258
- `renderReviewList()` — L5264
- `applyReviewOrder()` — L5313
- `updateAuthUI()` — L5340

**Init & session restore**  (from L5346)
- `hiddenNow(id, btn)` — L5357

**Tournaments**  (from L5371)
- `openAuthModal()` — L5427
- `closeAuthModal()` — L5428
- `showAuthView(view)` — L5429
- `setAuthSession(session, user, opts = {})` — L5437
- `openCompleteProfileModal()` — L5454
- `showCompleteProfileMode(mode, preselectPlayerId)` — L5469
- `populateClaimPicker(preselectPlayerId)` — L5477
- `submitClaimProfile()` — L5501
- `closeCompleteProfileModal()` — L5541
- `sanitizeNickname(raw)` — L5547
- `editDistance(a, b)` — L5552
- `checkForExistingPlayer(name, typedNickname, statusEl)` — L5574
- `submitCompleteProfile()` — L5629
- `finishRequestAndSignOut(message)` — L5705
- `doLogin()` — L5711
- `doNewPassword()` — L5764
- `doSignup()` — L5775
- `doConfirmSignup()` — L5792
- `doResendConfirmCode()` — L5823
- `doForgotPassword()` — L5834
- `doConfirmForgotPassword()` — L5849
- `doLogout()` — L5861
- `restoreSession()` — L5905
- `restoreTabFromHash()` — L5939
- `addManualTeamRow()` — L6005
- `collectManualTeams()` — L6041
- `loadTournamentGroupOptions()` — L6054
- `loadTournamentParticipantsChecklist()` — L6063
- `updateParticipantsCount()` — L6093
- `collectTournamentParticipants()` — L6105
- `loadTournamentsList()` — L6109
- `submitTournamentCreation(payload)` — L6116
- `collectAllEntities(t)` — L6275
- `getAllTeamEntities(t)` — L6291
- `renderTeamCompositionBars(t, containerId)` — L6309
- `populateSubstitutionSection(t)` — L6344
- `updateSubOldPlayerOptions()` — L6355

**Live scoring inside tournaments**  (from L6421)
- `formatGames(games)` — L6444
- `applyTournamentViewMode()` — L6451
- `matchTotals(match)` — L6457
- `truncateBracketName(name, maxChars = 22)` — L6465
- `renderBracketView(t)` — L6470
- `renderTournament(t)` — L6586
- `generateTournamentRecap(t)` — L6760
- `downloadTournamentImage()` — L6792
- `loadImg(src)` — L6819
- `sideVisuals(side)` — L6829
- `drawCard(x, y, w, match, isFinal)` — L6836
- `drawAvatars(ctx, x, y, side, isWinner)` — L6882
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L6901
- `roundRect(ctx, x, y, w, h, r)` — L6929
- `copyTournamentRecap()` — L6939
- `item_has_third_place(t)` — L6950
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L6954
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L6960
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L6979
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L6985
- `submitThirdPlaceScore(tournamentId)` — L7004
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L7010
- `getTournamentLiveLog(matchKey)` — L7033
- `tournamentLivePoint(matchKey, side, target)` — L7038
- `tournamentUndoPoint(matchKey, target)` — L7047
- `updateTournamentLiveDisplay(matchKey, target)` — L7053
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L7071
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L7080
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L7089
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L7098
- `applyTheme(theme)` — L7171
