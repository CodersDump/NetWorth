...
### Frontend (`frontend/js/app.js` — 10597 LOC, flat global script, ~446 functions)
...
- `playerRow(s, v)` — L8724
- `squadCard(sid)` — L8738
- `renderTieSection(title, ties, t, stageKind)` — L8782
- `renderTieCard(tie, t, stageKind)` — L8789
- `renderTieMatchRow(tie, m, idx, t, stageKind, iLeadA, iLead)` — L8808
- `draftPlayerPickerHtml(tournamentId, tieId, matchIndex, members)` — L8946
- `opts(selected)` — L8952
- `pickTiePlayer(tournamentId, tieId, matchIndex, playerI)` — L8972
- `pickTiePlayerPair(tournamentId, tieId, matchIndex, squadId)` — L8984
- `submitDraftTieScore(tournamentId, tieId, matchIndex, stageKi)` — L9000
- `submitDraftTieScoreDirect(tournamentId, tieId, matchIndex, stageKi)` — L9017
- `collectAllEntities(t)` — L9183
- `getAllTeamEntities(t)` — L9199
- `renderTeamCompositionBars(t, containerId)` — L9217
- `populateSubstitutionSection(t)` — L9252
- `updateSubOldPlayerOptions()` — L9263
- `formatGames(games)` — L9352
- `applyTournamentViewMode()` — L9359
- `matchTotals(match)` — L9375
- `truncateBracketName(name, maxChars = 22)` — L9383
- `renderBracketView(t)` — L9388
- `renderDraftBracketGroupsPanel(t)` — L9516
- `renderDraftBracketView(t)` — L9552
- `renderTournament(t)` — L9683
- `generateTournamentRecap(t)` — L9858
- `downloadTournamentImage()` — L9890
- `loadImg(src)` — L9917
- `sideVisuals(side)` — L9927
- `drawCard(x, y, w, match, isFinal)` — L9934
- `drawAvatars(ctx, x, y, side, isWinner)` — L9980
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L9999
- `roundRect(ctx, x, y, w, h, r)` — L10027
- `downloadDraftShareImage()` — L10043
- `loadImg(src)` — L10050
- `sideAvatars(squadId)` — L10099
- `drawSide(side, sx, sy, isWinner)` — L10180
- `copyTournamentRecap()` — L10236
- `item_has_third_place(t)` — L10247
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L10251
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L10257
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L10294
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L10300
- `submitThirdPlaceScore(tournamentId)` — L10327
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L10333
- `getTournamentLiveLog(matchKey)` — L10364
- `tournamentLivePoint(matchKey, side, target)` — L10369
- `tournamentUndoPoint(matchKey, target)` — L10378
- `updateTournamentLiveDisplay(matchKey, target)` — L10384
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L10402
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L10416
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L10425
- `finishDraftTieLiveGame(matchKey, tournamentId, tieId, matchInde)` — L10443
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L10452
- `activateTab(tabName)` — L10476
- `jumpToRecordMatch()` — L10572
- `applyTheme(theme)` — L10578
