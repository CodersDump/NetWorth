...
### Frontend (`frontend/js/app.js` — 7294 LOC, flat global script, ~296 functions)
...
- `line(n, o, r, p)` — L5104
- `done()` — L5114
- `fallbackCopy(text, cb)` — L5120
- `renderInsights()` — L5129
- `saveFinanceSettings()` — L5202
- `loadPublicWalkins()` — L5239
- `loadReviewDay()` — L5318

**Init & session restore**  (from L5346)
- `reviewOrderChanged()` — L5362
- `renderReviewList()` — L5368

**Tournaments**  (from L5371)
- `applyReviewOrder()` — L5417
- `updateAuthUI()` — L5444
- `hiddenNow(id, btn)` — L5461
- `openAuthModal()` — L5531
- `closeAuthModal()` — L5532
- `showAuthView(view)` — L5533
- `setAuthSession(session, user, opts = {})` — L5541
- `openCompleteProfileModal()` — L5558
- `showCompleteProfileMode(mode, preselectPlayerId)` — L5573
- `populateClaimPicker(preselectPlayerId)` — L5581
- `submitClaimProfile()` — L5605
- `closeCompleteProfileModal()` — L5645
- `sanitizeNickname(raw)` — L5651
- `editDistance(a, b)` — L5656
- `checkForExistingPlayer(name, typedNickname, statusEl)` — L5678
- `submitCompleteProfile()` — L5733
- `finishRequestAndSignOut(message)` — L5809
- `doLogin()` — L5815
- `doNewPassword()` — L5868
- `doSignup()` — L5879
- `doConfirmSignup()` — L5896
- `doResendConfirmCode()` — L5927
- `doForgotPassword()` — L5938
- `doConfirmForgotPassword()` — L5953
- `doLogout()` — L5965
- `restoreSession()` — L6009
- `restoreTabFromHash()` — L6043
- `addManualTeamRow()` — L6109
- `collectManualTeams()` — L6145
- `loadTournamentGroupOptions()` — L6158
- `loadTournamentParticipantsChecklist()` — L6167
- `updateParticipantsCount()` — L6197
- `collectTournamentParticipants()` — L6209
- `loadTournamentsList()` — L6213
- `submitTournamentCreation(payload)` — L6220
- `collectAllEntities(t)` — L6379
- `getAllTeamEntities(t)` — L6395
- `renderTeamCompositionBars(t, containerId)` — L6413

**Live scoring inside tournaments**  (from L6421)
- `populateSubstitutionSection(t)` — L6448
- `updateSubOldPlayerOptions()` — L6459
- `formatGames(games)` — L6548
- `applyTournamentViewMode()` — L6555
- `matchTotals(match)` — L6561
- `truncateBracketName(name, maxChars = 22)` — L6569
- `renderBracketView(t)` — L6574
- `renderTournament(t)` — L6690
- `generateTournamentRecap(t)` — L6864
- `downloadTournamentImage()` — L6896
- `loadImg(src)` — L6923
- `sideVisuals(side)` — L6933
- `drawCard(x, y, w, match, isFinal)` — L6940
- `drawAvatars(ctx, x, y, side, isWinner)` — L6986
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L7005
- `roundRect(ctx, x, y, w, h, r)` — L7033
- `copyTournamentRecap()` — L7043
- `item_has_third_place(t)` — L7054
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L7058
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L7064
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L7083
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L7089
- `submitThirdPlaceScore(tournamentId)` — L7108
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L7114
- `getTournamentLiveLog(matchKey)` — L7137
- `tournamentLivePoint(matchKey, side, target)` — L7142
- `tournamentUndoPoint(matchKey, target)` — L7151
- `updateTournamentLiveDisplay(matchKey, target)` — L7157
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L7175
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L7184
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L7193
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L7202
- `applyTheme(theme)` — L7275
