...
### Frontend (`frontend/js/app.js` — 7249 LOC, flat global script, ~290 functions)
...
- `renderInsights()` — L5085
- `saveFinanceSettings()` — L5157
- `loadPublicWalkins()` — L5194
- `loadReviewDay()` — L5273
- `reviewOrderChanged()` — L5317
- `renderReviewList()` — L5323

**Tournaments**  (from L5371)
- `applyReviewOrder()` — L5372
- `updateAuthUI()` — L5399
- `hiddenNow(id, btn)` — L5416
- `openAuthModal()` — L5486
- `closeAuthModal()` — L5487
- `showAuthView(view)` — L5488
- `setAuthSession(session, user, opts = {})` — L5496
- `openCompleteProfileModal()` — L5513
- `showCompleteProfileMode(mode, preselectPlayerId)` — L5528
- `populateClaimPicker(preselectPlayerId)` — L5536
- `submitClaimProfile()` — L5560
- `closeCompleteProfileModal()` — L5600
- `sanitizeNickname(raw)` — L5606
- `editDistance(a, b)` — L5611
- `checkForExistingPlayer(name, typedNickname, statusEl)` — L5633
- `submitCompleteProfile()` — L5688
- `finishRequestAndSignOut(message)` — L5764
- `doLogin()` — L5770
- `doNewPassword()` — L5823
- `doSignup()` — L5834
- `doConfirmSignup()` — L5851
- `doResendConfirmCode()` — L5882
- `doForgotPassword()` — L5893
- `doConfirmForgotPassword()` — L5908
- `doLogout()` — L5920
- `restoreSession()` — L5964
- `restoreTabFromHash()` — L5998
- `addManualTeamRow()` — L6064
- `collectManualTeams()` — L6100
- `loadTournamentGroupOptions()` — L6113
- `loadTournamentParticipantsChecklist()` — L6122
- `updateParticipantsCount()` — L6152
- `collectTournamentParticipants()` — L6164
- `loadTournamentsList()` — L6168
- `submitTournamentCreation(payload)` — L6175
- `collectAllEntities(t)` — L6334
- `getAllTeamEntities(t)` — L6350
- `renderTeamCompositionBars(t, containerId)` — L6368
- `populateSubstitutionSection(t)` — L6403
- `updateSubOldPlayerOptions()` — L6414

**Live scoring inside tournaments**  (from L6421)
- `formatGames(games)` — L6503
- `applyTournamentViewMode()` — L6510
- `matchTotals(match)` — L6516
- `truncateBracketName(name, maxChars = 22)` — L6524
- `renderBracketView(t)` — L6529
- `renderTournament(t)` — L6645
- `generateTournamentRecap(t)` — L6819
- `downloadTournamentImage()` — L6851
- `loadImg(src)` — L6878
- `sideVisuals(side)` — L6888
- `drawCard(x, y, w, match, isFinal)` — L6895
- `drawAvatars(ctx, x, y, side, isWinner)` — L6941
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L6960
- `roundRect(ctx, x, y, w, h, r)` — L6988
- `copyTournamentRecap()` — L6998
- `item_has_third_place(t)` — L7009
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L7013
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L7019
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L7038
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L7044
- `submitThirdPlaceScore(tournamentId)` — L7063
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L7069
- `getTournamentLiveLog(matchKey)` — L7092
- `tournamentLivePoint(matchKey, side, target)` — L7097
- `tournamentUndoPoint(matchKey, target)` — L7106
- `updateTournamentLiveDisplay(matchKey, target)` — L7112
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L7130
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L7139
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L7148
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L7157
- `applyTheme(theme)` — L7230
