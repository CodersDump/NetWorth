...
### Frontend (`frontend/js/app.js` — 10852 LOC, flat global script, ~452 functions)
...
- `draftPlayerPickerHtml(tournamentId, tieId, matchIndex, members)` — L9048
- `opts(selected)` — L9054
- `pickTiePlayer(tournamentId, tieId, matchIndex, playerI)` — L9074
- `pickTiePlayerPair(tournamentId, tieId, matchIndex, squadId)` — L9086
- `submitDraftTieScore(tournamentId, tieId, matchIndex, stageKi)` — L9102
- `submitDraftTieScoreDirect(tournamentId, tieId, matchIndex, stageKi)` — L9119
- `collectAllEntities(t)` — L9285
- `getAllTeamEntities(t)` — L9301
- `renderTeamCompositionBars(t, containerId)` — L9319
- `populateSubstitutionSection(t)` — L9354
- `updateSubOldPlayerOptions()` — L9365
- `formatGames(games)` — L9454
- `applyTournamentViewMode()` — L9461
- `matchTotals(match)` — L9477
- `truncateBracketName(name, maxChars = 22)` — L9485
- `renderBracketView(t)` — L9490
- `renderDraftBracketGroupsPanel(t)` — L9618
- `renderDraftBracketView(t)` — L9654
- `renderTournament(t)` — L9785
- `generateTournamentRecap(t)` — L9960
- `downloadTournamentImage()` — L9992
- `loadImg(src)` — L10019
- `sideVisuals(side)` — L10029
- `drawCard(x, y, w, match, isFinal)` — L10036
- `drawAvatars(ctx, x, y, side, isWinner)` — L10082
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L10101
- `roundRect(ctx, x, y, w, h, r)` — L10129
- `downloadDraftShareImage()` — L10145
- `loadImg(src)` — L10152
- `sideAvatars(squadId)` — L10201
- `drawSide(side, sx, sy, isWinner)` — L10282
- `downloadDraftLeaderboardImage()` — L10363
- `loadImg(src)` — L10372
- `presetKeyFor(bannerCss)` — L10381
- `copyTournamentRecap()` — L10491
- `item_has_third_place(t)` — L10502
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L10506
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L10512
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L10549
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L10555
- `submitThirdPlaceScore(tournamentId)` — L10582
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L10588
- `getTournamentLiveLog(matchKey)` — L10619
- `tournamentLivePoint(matchKey, side, target)` — L10624
- `tournamentUndoPoint(matchKey, target)` — L10633
- `updateTournamentLiveDisplay(matchKey, target)` — L10639
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L10657
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L10671
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L10680
- `finishDraftTieLiveGame(matchKey, tournamentId, tieId, matchInde)` — L10698
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L10707
- `activateTab(tabName)` — L10731
- `jumpToRecordMatch()` — L10827
- `applyTheme(theme)` — L10833
