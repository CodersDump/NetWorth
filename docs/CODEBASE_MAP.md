...
### Frontend (`frontend/js/app.js` — 7025 LOC, flat global script, ~279 functions)
...
- `manageGroupSlots(groupId)` — L4396
- `assignSlotMembers(groupId, slotEnc)` — L4414
- `transferGroupOwnership(groupId)` — L4442
- `setGroupPayee(groupId)` — L4461
- `requestFinanceAccess()` — L4484
- `financeUnlock()` — L4501
- `loadFinanceSummary()` — L4539
- `loadFinanceExpenses()` — L4560
- `resetExpenseEdit()` — L4598
- `addFinanceExpense()` — L4605
- `loadFinanceMembers()` — L4624

**Match review & reorder (SuperAdmin)**  (from L4649)
- `renderBulkRosterList()` — L4709
- `bulkAddFromRoster()` — L4723
- `copyPreviousMonthMembers()` — L4738
- `addFinanceMember()` — L4774

**Auth UI (Cognito login/signup/session)**  (from L4780)
- `loadFinanceWalkins()` — L4795
- `addFinanceWalkin()` — L4822
- `loadFinanceInsights()` — L4860
- `renderInsights()` — L4874
- `saveFinanceSettings()` — L4946
- `loadPublicWalkins()` — L4983
- `loadReviewDay()` — L5060
- `reviewOrderChanged()` — L5104
- `renderReviewList()` — L5110
- `applyReviewOrder()` — L5159
- `updateAuthUI()` — L5186
- `openAuthModal()` — L5263
- `closeAuthModal()` — L5264
- `showAuthView(view)` — L5265
- `setAuthSession(session, user, opts = {})` — L5273
- `openCompleteProfileModal()` — L5290
- `showCompleteProfileMode(mode, preselectPlayerId)` — L5305
- `populateClaimPicker(preselectPlayerId)` — L5313
- `submitClaimProfile()` — L5337

**Tournaments**  (from L5371)
- `closeCompleteProfileModal()` — L5377
- `sanitizeNickname(raw)` — L5383
- `editDistance(a, b)` — L5388
- `checkForExistingPlayer(name, typedNickname, statusEl)` — L5410
- `submitCompleteProfile()` — L5465
- `finishRequestAndSignOut(message)` — L5541
- `doLogin()` — L5547
- `doNewPassword()` — L5600
- `doSignup()` — L5611
- `doConfirmSignup()` — L5628
- `doResendConfirmCode()` — L5659
- `doForgotPassword()` — L5670
- `doConfirmForgotPassword()` — L5685
- `doLogout()` — L5697
- `restoreSession()` — L5741
- `restoreTabFromHash()` — L5775
- `addManualTeamRow()` — L5841
- `collectManualTeams()` — L5877
- `loadTournamentGroupOptions()` — L5890
- `loadTournamentParticipantsChecklist()` — L5899
- `updateParticipantsCount()` — L5929
- `collectTournamentParticipants()` — L5941
- `loadTournamentsList()` — L5945
- `submitTournamentCreation(payload)` — L5952
- `collectAllEntities(t)` — L6111
- `getAllTeamEntities(t)` — L6127
- `renderTeamCompositionBars(t, containerId)` — L6145
- `populateSubstitutionSection(t)` — L6180
- `updateSubOldPlayerOptions()` — L6191
- `formatGames(games)` — L6280
- `applyTournamentViewMode()` — L6287
- `matchTotals(match)` — L6293
- `truncateBracketName(name, maxChars = 22)` — L6301
- `renderBracketView(t)` — L6306

**Live scoring inside tournaments**  (from L6421)
- `renderTournament(t)` — L6422
- `generateTournamentRecap(t)` — L6596
- `downloadTournamentImage()` — L6628
- `loadImg(src)` — L6655
- `sideVisuals(side)` — L6665
- `drawCard(x, y, w, match, isFinal)` — L6672
- `drawAvatars(ctx, x, y, side, isWinner)` — L6718
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L6737
- `roundRect(ctx, x, y, w, h, r)` — L6765
- `copyTournamentRecap()` — L6775
- `item_has_third_place(t)` — L6786
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L6790
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L6796
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L6815
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L6821
- `submitThirdPlaceScore(tournamentId)` — L6840
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L6846
- `getTournamentLiveLog(matchKey)` — L6869
- `tournamentLivePoint(matchKey, side, target)` — L6874
- `tournamentUndoPoint(matchKey, target)` — L6883
- `updateTournamentLiveDisplay(matchKey, target)` — L6889
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L6907
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L6916
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L6925
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L6934
- `applyTheme(theme)` — L7006
