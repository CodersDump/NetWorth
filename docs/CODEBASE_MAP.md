...
### Frontend (`frontend/js/app.js` — 7221 LOC, flat global script, ~288 functions)
...
- `loadProfilePartnershipsAndRadar(playerId)` — L3655
- `loadProfileHeadToHead(playerId)` — L3704
- `loadProfileWithPartner(playerId)` — L3728
- `partnerGamesGoto(p)` — L3760
- `renderPartnerGames()` — L3762
- `skeletonHTML(lines = 3)` — L3795
- `showProfileSkeletons()` — L3802
- `renderXpPanel(player)` — L3815
- `xpForLevel(n)` — L3823
- `updateHeaderCoins()` — L3849
- `loadProfile()` — L3861
- `refreshProfile()` — L3886
- `refreshProfileIfShowing(affectedPlayerIds)` — L3903
- `renderPartnerRadar(data, highlightTournament, svgId = 'rada)` — L3924

**UPI payment card**  (from L3941)
- `loadHistory()` — L3980
- `renderHistory(data)` — L3998

**Finance tab (view-key + role gated)**  (from L3999)
- `loadBadges()` — L4059
- `renderBadges(data)` — L4077
- `loadDiversity()` — L4110
- `renderDiversity(data)` — L4128
- `playerLabelById(playerId, fallbackName)` — L4149
- `playerLabelsById(playerIds, fallbackNames)` — L4153
- `loadHallOfFame()` — L4159
- `renderHallOfFame(data)` — L4181
- `loadAttendance()` — L4265
- `renderAttendance(data)` — L4284
- `refreshUpiCard()` — L4303
- `renderUpiCard()` — L4315
- `imageServiceFallback()` — L4337
- `xpVisible()` — L4365
- `applyFinanceRoleVisibility()` — L4371
- `finQS(extra)` — L4392
- `financeBaseUrl()` — L4403
- `finPost(path, method, bodyObj)` — L4407
- `populateFinanceSlots(group)` — L4431
- `populateFinanceGroups()` — L4442
- `reloadFinanceForGroup()` — L4470
- `tryAutoFinanceUnlock()` — L4475
- `myFinanceGroups()` — L4500
- `populateMyDuesGroups()` — L4505
- `loadMyDues(groupId)` — L4523
- `manageGroupSlots(groupId)` — L4568
- `assignSlotMembers(groupId, slotEnc)` — L4586
- `transferGroupOwnership(groupId)` — L4614
- `setGroupPayee(groupId)` — L4633

**Match review & reorder (SuperAdmin)**  (from L4649)
- `requestFinanceAccess()` — L4656
- `financeUnlock()` — L4673
- `loadFinanceSummary()` — L4711
- `loadFinanceExpenses()` — L4732
- `resetExpenseEdit()` — L4770
- `addFinanceExpense()` — L4777

**Auth UI (Cognito login/signup/session)**  (from L4780)
- `loadFinanceMembers()` — L4796
- `scheduleMemberReload()` — L4885
- `renderBulkRosterList()` — L4893
- `bulkAddFromRoster()` — L4907
- `copyPreviousMonthMembers()` — L4922
- `addFinanceMember()` — L4958
- `loadFinanceWalkins()` — L4979
- `addFinanceWalkin()` — L5006
- `loadFinanceInsights()` — L5044
- `renderInsights()` — L5058
- `saveFinanceSettings()` — L5130
- `loadPublicWalkins()` — L5167
- `loadReviewDay()` — L5245
- `reviewOrderChanged()` — L5289
- `renderReviewList()` — L5295
- `applyReviewOrder()` — L5344

**Tournaments**  (from L5371)
- `updateAuthUI()` — L5371
- `hiddenNow(id, btn)` — L5388
- `openAuthModal()` — L5458
- `closeAuthModal()` — L5459
- `showAuthView(view)` — L5460
- `setAuthSession(session, user, opts = {})` — L5468
- `openCompleteProfileModal()` — L5485
- `showCompleteProfileMode(mode, preselectPlayerId)` — L5500
- `populateClaimPicker(preselectPlayerId)` — L5508
- `submitClaimProfile()` — L5532
- `closeCompleteProfileModal()` — L5572
- `sanitizeNickname(raw)` — L5578
- `editDistance(a, b)` — L5583
- `checkForExistingPlayer(name, typedNickname, statusEl)` — L5605
- `submitCompleteProfile()` — L5660
- `finishRequestAndSignOut(message)` — L5736
- `doLogin()` — L5742
- `doNewPassword()` — L5795
- `doSignup()` — L5806
- `doConfirmSignup()` — L5823
- `doResendConfirmCode()` — L5854
- `doForgotPassword()` — L5865
- `doConfirmForgotPassword()` — L5880
- `doLogout()` — L5892
- `restoreSession()` — L5936
- `restoreTabFromHash()` — L5970
- `addManualTeamRow()` — L6036
- `collectManualTeams()` — L6072
- `loadTournamentGroupOptions()` — L6085
- `loadTournamentParticipantsChecklist()` — L6094
- `updateParticipantsCount()` — L6124
- `collectTournamentParticipants()` — L6136
- `loadTournamentsList()` — L6140
- `submitTournamentCreation(payload)` — L6147
- `collectAllEntities(t)` — L6306
- `getAllTeamEntities(t)` — L6322
- `renderTeamCompositionBars(t, containerId)` — L6340
- `populateSubstitutionSection(t)` — L6375
- `updateSubOldPlayerOptions()` — L6386

**Live scoring inside tournaments**  (from L6421)
- `formatGames(games)` — L6475
- `applyTournamentViewMode()` — L6482
- `matchTotals(match)` — L6488
- `truncateBracketName(name, maxChars = 22)` — L6496
- `renderBracketView(t)` — L6501
- `renderTournament(t)` — L6617
- `generateTournamentRecap(t)` — L6791
- `downloadTournamentImage()` — L6823
- `loadImg(src)` — L6850
- `sideVisuals(side)` — L6860
- `drawCard(x, y, w, match, isFinal)` — L6867
- `drawAvatars(ctx, x, y, side, isWinner)` — L6913
- `paintTeam(ctx, x, y, w, h, side, fallback)` — L6932
- `roundRect(ctx, x, y, w, h, r)` — L6960
- `copyTournamentRecap()` — L6970
- `item_has_third_place(t)` — L6981
- `submitGroupScore(tournamentId, subgroup, fixtureId)` — L6985
- `submitGroupScoreDirect(tournamentId, subgroup, fixtureId, score)` — L6991
- `submitKnockoutScore(tournamentId, roundIndex, matchIndex)` — L7010
- `submitKnockoutScoreDirect(tournamentId, roundIndex, matchIndex, sc)` — L7016
- `submitThirdPlaceScore(tournamentId)` — L7035
- `submitThirdPlaceScoreDirect(tournamentId, score_a, score_b, override)` — L7041
- `getTournamentLiveLog(matchKey)` — L7064
- `tournamentLivePoint(matchKey, side, target)` — L7069
- `tournamentUndoPoint(matchKey, target)` — L7078
- `updateTournamentLiveDisplay(matchKey, target)` — L7084
- `finishGroupLiveGame(matchKey, tournamentId, subgroup, fixtur)` — L7102
- `finishKnockoutLiveGame(matchKey, tournamentId, roundIndex, matc)` — L7111
- `finishThirdPlaceLiveGame(matchKey, tournamentId)` — L7120
- `renderLiveScoreControls(matchKey, target, finishCallExpr, nameA,)` — L7129
- `applyTheme(theme)` — L7202
